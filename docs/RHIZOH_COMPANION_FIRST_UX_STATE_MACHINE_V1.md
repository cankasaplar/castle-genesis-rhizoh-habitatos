...
**Kaynak gerçeklik (bugün):** `CastleShellRouter` tüm path’leri tek `AppRhizoh528`’e verir; `productSurface` `SET_PRODUCT_SURFACE` ile güncellenir ve izinli küme **PR-1:** `core | world | demo` (yürütme alanı). Sunum: `spatialViewMode` (`globe | real_map | spiral | studio_layered`). Alt bar kısayol id’leri (`world` … `profile`) `deriveShellHighlightId` ile path + surface’tan türetilir (`castleRuntimeShellModelV0.js`). Alt bar: `UnifiedProductShellBar.jsx`.
