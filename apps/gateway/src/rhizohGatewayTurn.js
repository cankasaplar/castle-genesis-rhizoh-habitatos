 ...
 */
async function executeRhizohGatewayTurn(traceId, input) {
  ...
  try {
    ...
    const thetaProbe = extractConstitutionalThetaFromContext(fullContext);
    const preThetaReplyChars = String(llmRaw.reply || "").length;
    result = applyConstitutionalThetaLlmFilter(llmRaw, thetaProbe);
    const thetaFb = estimateConstitutionalThetaLlmStressFeedback(result);
    Object.assign(result, {
      constitutionalThetaStressBump: thetaFb.stressBump,
      constitutionalThetaStressFeedbackHint: thetaFb
    });
    const baseLedger =
      llmRaw.rhizohCompressionLedger && typeof llmRaw.rhizohCompressionLedger === "object"
        ? llmRaw.rhizohCompressionLedger
        : {};
    result.rhizohCompressionLedger = {
      ...baseLedger,
      preThetaReplyChars,
      postThetaReplyChars: String(result.reply || "").length,
      constitutionalThetaPhase: result.constitutionalThetaPhase ?? null,
      constitutionalThetaFilter: result.constitutionalThetaFilter ?? null
    };
    ...
    applyRhizohProductionLayer(result, {
      traceId,
      turnLatencyMs,
      turnT0: t0,
      fullContext,
    ...
    recordRhizohLlmTurnCompleted({
      turnLatencyMs: turnLatencyMs || Math.max(0, Date.now() - t0),
      decisionAction: result?.rhizohProduction?.decision?.action,
      ok: okStruct
    });
  }
}
