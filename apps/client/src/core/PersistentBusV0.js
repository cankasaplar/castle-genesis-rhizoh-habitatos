...
import { CODEX_EVENT_TYPE_V0 } from "./codexReducerV0.js";
...
export async function persistentCodexEmitV0(type, payload = {}) {
  ...
  if (canPersistUserTopologyN12V0()) {
    const pushOut = await pushSimulationEventV0(eventType, payload, { cycle });
    if (pushOut.ok) {
      persisted = true;
      event = pushOut.event;
      await applyPersistedSideEffectsV0(eventType, payload, event, cycle);
