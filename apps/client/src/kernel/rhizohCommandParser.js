...
export function detectCastleIntentWithoutCoords(message) {
  ...
  if (
    /kendi\s+kalemi|kişisel\s+kale|kisisel\s+kale|personal\s+castle|sovereign\s+castle|kalemi\s+kur|kale\s+kur|kale\s+oluştur|kale\s+olustur|castle\s+create|create\s+castle|build\s+castle|rhizoh\s+kale\s+kur|locate\s*&\s*spawn/i.test(
      m
    )
  ) {
