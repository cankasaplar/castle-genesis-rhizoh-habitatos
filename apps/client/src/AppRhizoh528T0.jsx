...
import { readRhizohWorldSystemModeV0, RHIZOH_WORLD_SYSTEM_MODE_V0 } from "./rhizoh/runtime/rhizohWorldSystemModeV0.js";
import {
  bootstrapRhizohDomainGateV0,
  RHIZOH_DOMAIN_ID_V0
} from "./rhizoh/runtime/rhizohDomainGateV0.js";
import {
  dispatchDomainCapabilityV0,
  runDomainGateForPathV0,
  shareWorldInviteV0,
...
} from "./rhizoh/experience/rhizohExperienceSessionContextV0.js";
...
} from "./rhizoh/runtime/voiceInstantAckV0.js";
import {
...
import {
  ...
  resumeVoiceAudioContextV0,
  voiceSttEmptyPromptTrV0
} from "./rhizoh/runtime/voiceSttSessionGuardV0.js";
import {
  getVoiceGestureAdvisoryV0,
  getVoiceGestureTrustV0,
  getVoiceGestureAnchorSnapshotV0,
  installVoiceUserGestureListenersV0,
  publishVoiceGestureAnchorToWindowV0,
  stampVoiceUserGestureV0
} from "./rhizoh/runtime/voiceUserGestureAnchorV0.js";
import {
  getVoiceSttTelemetrySnapshotV0,
  getVoiceInitSnapshotV0,
  installVoiceSttTelemetryV0,
  noteVoiceSttEventV0
} from "./rhizoh/runtime/voiceSttTelemetryV0.js";
import {
...
} from "./rhizoh/runtime/voiceSttChromeConfigV0.js";
import {
  createVoiceEngineV3TurnBridgeV0,
  isVoiceEngineV3EnabledV0
} from "./rhizoh/runtime/voiceEngineV3/voiceEngineV3TurnBridgeV0.js";
import { sanitizeVoiceTranscriptForDispatchV3 } from "./rhizoh/runtime/voiceEngineV3/voiceTranscriptSanityV3.js";
import {
  evaluateVoiceTurnAcceptanceV0,
  voiceTurnAcceptanceLogDetailV0
} from "./rhizoh/runtime/voiceTurnAcceptanceGateV0.js";
import {
...
} from "./rhizoh/runtime/voiceBehavioralCommitmentV0.js";
import { VOICE_DIRECTED_SPEECH_BAND } from "./rhizoh/runtime/voiceDirectedSpeechObservationV0.js";
import {
...
import {
  ...
  resolveRhizohProductPathV0
} from "./rhizoh/product/rhizohProductTopologyV0.js";
import {
...
/** ─░stemci `options.maxTokens` ÔÇö gateway `RHIZOH_GENERATION_MODES` ile ayn─▒ anahtarlar. */
...
const RHIZOH_GENERATION_MODE_UI = [
  { id: "FAST_DIALOGUE", label: "H─▒zl─▒", sub: "K─▒sa cevap" },
  { id: "STANDARD", label: "Standart", sub: "Dengeli sohbet" },
  { id: "REFLECTIVE", label: "Yans─▒t─▒c─▒", sub: "Derin / ├Âl├ğ├╝l├╝" },
  { id: "NARRATIVE", label: "Anlat─▒", sub: "Uzun hik├óye" },
...
/** L0ÔÇôL13 + Rhizoh command plane (L13 = Robotics Mechanics bridge layer) */
const LAYER_SPECS = [
...
const LAYER_UI_PROFILES = {
  0: {
    ...
    detail: "ECS hareketleri, damping, sabit ad─▒m sim├╝lasyon do─şrulama",
    reality: "GLOBE",
    camera: "ORBIT",
    satellite: false,
  ...
  1: {
    mission: "Spatial hash yo─şunlu─şu ve komsuluk taramas─▒",
    detail: "Yak─▒n ajan k├╝meleri ve grid tabanl─▒ davran─▒┼ş etkilerini izle",
    reality: "GLOBE",
    camera: "DRONE",
    satellite: false,
  ...
  2: {
    ...
    detail: "Arketip bazl─▒ davran─▒┼ş ├ğe┼şitlili─şi ve g├Ârev da─ş─▒l─▒m─▒",
    reality: "GLOBE",
    camera: "DRONE",
    satellite: false,
  ...
  3: {
    mission: "MMO presence ak─▒┼ş─▒ ve canl─▒ pop├╝lasyon",
    detail: "E┼şzamanl─▒ ajan yo─şunlu─şu ve etkile┼şim s├╝reklili─şi",
    reality: "GLOBE",
    camera: "ORBIT",
    satellite: true,
  ...
  4: {
    mission: "Uydu sinyal katman─▒ do─şrulamas─▒",
    detail: "Katmanlar aras─▒ tarama, feed ve telemetri g├Âr├╝n├╝rl├╝─ş├╝",
    reality: "REAL_MAP",
    camera: "ORBIT",
    satellite: true,
  ...
  5: {
    mission: "GreenRoom canl─▒ yay─▒n ve medya rezonans─▒",
    detail: "Canl─▒ ba─şlant─▒, stream tetikleyicileri ve media k├Âpr├╝s├╝",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: false,
  ...
  6: {
    mission: "Swarm/Squad formasyon davran─▒┼ş─▒",
    detail: "Squad intent ve y├Âr├╝nge ayr─▒┼şmas─▒n─▒n sahnede izlenmesi",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: false,
    quickActions: ["SUMMON SQUAD", "ACTIVATE SWARM"],
  ...
  7: {
    mission: "Ghost pet evrimi ve owner ba─ş─▒",
    detail: "Pet progression, ba─ş g├╝c├╝ ve mikro s├╝r├╝ etkileri",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: false,
  ...
  8: {
    mission: "Procedural city zihin katman─▒",
    detail: "District enerji ak─▒┼ş─▒, tower etkisi ve ┼şehir dokusu",
    reality: "REAL_MAP",
    camera: "ORBIT",
    satellite: false,
  ...
  9: {
    mission: "Event mesh darbeleri ve neden-sonu├ğ zinciri",
    detail: "Katmanlar aras─▒ event pulse izleme ve yo─şunluk kontrol├╝",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: true,
  ...
  10: {
    mission: "Komut orkestrasyonu ve operasyon kontrol├╝",
    detail: "Rhizoh komut hatt─▒ ile ger├ğek zamanl─▒ senaryo y├Ânetimi",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: true,
  ...
  11: {
    mission: "Academics ya┼şam d├Âng├╝s├╝ ve mezuniyet",
    detail: "Curriculum, exam, graduation queue ve ├Â─şretici etkile┼şim",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: false,
    quickActions: ["RUN CURRICULUM", "EXAM MODE"],
  ...
  12: {
    ...
    detail: "Boot sequence, Chronos clock ve manifest sa─şl─▒─ş─▒",
    reality: "REAL_MAP",
    camera: "ORBIT",
    satellite: true,
  ...
  13: {
    mission: "Robotics-mechanics entegrasyonu ve live kontrol k├Âpr├╝s├╝",
    detail: "Kullan─▒c─▒ robotik/drone sistemlerini Rhizoh Brain ile ba─şlar, komut-plan d├Âng├╝s├╝ ├╝retir",
    reality: "REAL_MAP",
    camera: "DRONE",
    satellite: true,
...
/** District zoning (procedural city mind) ÔÇö + Castle Academics */
...
/** Boids / hayalet grid kom┼şu ├╝st s─▒n─▒r─▒ (O(N┬ÀK) yerine sabit K). */
...
/** Studio oturumlar─▒ + transcript meta alanlar─▒ndan oynat─▒labilir URL listesi ├╝retir. */
function mergeMediaPlaylistFromGateway(sessions, transcripts) {
  ...
  for (const s of sessions || []) {
    ...
    if (ingest) push(`pub-${s.id}`, `${s.protocol || "PUB"} ┬À ${String(s.target || s.status || "").slice(0, 28)}`, classifyMediaUrl(ingest), ingest, "publish");
  }
  for (const t of transcripts || []) {
    ...
    for (const p of cand) {
      ...
      push(
        `trx-${t.id || "x"}-${k}`,
        `${t.eventType || "event"} ┬À ${String(t.text || "").slice(0, 36)}`,
...
/** E1: GreenRoom capability ÔÇö transcript + Castle Library memory commit (gateway). */
...
const LIVE_FIELD_REACTIONS = [
  ...
  () => "ÔÇóÔÇóÔÇó ripple echoes increased",
...
function formatRecentRealityLines(meta, firstName) {
  ...
  return log.slice(-12).map((e) => {
    if (e?.note) {
      return String(e.note).replace(/^Kullan─▒c─▒\b/, who);
    }
    const src = e?.source || "APP";
    const dm = Number.isFinite(e?.durationMs) ? ` (${e.durationMs}ms)` : "";
    if (e?.to === "REAL_MAP" && e?.from === "GLOBE") return `${who} ${src} ile REAL_MAP'e ge├ğti${dm}`;
    if (e?.to === "GLOBE" && e?.from === "REAL_MAP") return `${who} ${src} ile GLOBE'a d├Ând├╝${dm}`;
    return `${who} ${src}: ${e?.from} ÔåÆ ${e?.to}${dm}`;
  });
}
...
class CityMind {
  ...
  /** Ki┼şisel kale ÔÇö districtOwner + tip; koordinat CityMind d─▒┼ş─▒nda saklan─▒r (continuity). */
...
function syncGhostPetToCastle(lat, lon) {
  try {
    ...
    log.push({
      at: Date.now(),
      note: `Ki┼şisel kale SEED ┬À ${Number(lat).toFixed(4)}, ${Number(lon).toFixed(4)}`,
      source: "DSL_SPAWN"
    });
...
/**
 * DSL k├Âpr├╝s├╝: CityMind + Event Mesh + s├╝reklilik + REAL_MAP + Cesium flyToCustom.
 * @param {{ verb: string, args: Record<string, string> }} parsed
 */
async function applyPersonalCastleDsl(parsed) {
  if (!parsed || !parsed.verb) return { ok: false, reply: "Ge├ğersiz DSL." };
  if (parsed.verb === "SPAWN_CASTLE") {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
      return {
        ok: false,
        reply:
          "SPAWN CASTLE i├ğin --lat ve --lon say─▒sal olmal─▒ (├Ârn. --lat 41.0082 --lon 28.9784). Panelden ┬½Konum ile kur┬╗ kullanabilirsiniz."
      };
    }
    ...
    return {
      ok: true,
      reply: `Kale y├╝kseltildi (${type}) ┬À b├Âlge #${zone.slot} ┬À sahip ${owner}. Ghost devriye; SEED a┼şamas─▒nda Event Mesh rezonans─▒ ve ├ğevre ajanlar─▒ndan pasif XP ak─▒┼ş─▒ ba┼şlar.`,
  ...
  if (parsed.verb === "WAKE_TCEE") {
    ...
    uiStore.dispatch({
      ...
      payload: {
        ts: new Date().toLocaleTimeString(),
        type: "SYS",
        data: `TCEE WAKE ┬À Phase B seal (${reason}) ┬À L10 ┬À memoryClockEpoch=${nextMeta.tceeBoot?.memoryClockEpoch ?? ""}`
      }
    });
    return {
      ok: true,
      reply:
        "TCEE WAKE tamam (Phase B): kimlik manifoldu m├╝h├╝rlendi, haf─▒za saati epoch ba┼şlad─▒, tam CSPE + recallÔåÆidentity a├ğ─▒k. ├ûnceki faz pre-breath yaln─▒zca pasif sens├Âr/nab─▒z.",
  ...
  if (parsed.verb === "SLEEP_TCEE") {
    ...
    uiStore.dispatch({
      ...
      payload: {
        ts: new Date().toLocaleTimeString(),
        type: "SYS",
        data: "TCEE HIBERNATE ┬À pre_breath (recallÔåÆidentity kapal─▒)"
      }
    });
    return {
      ok: true,
      reply:
        "TCEE HIBERNATE: pre-breath. Pasif alan nabz─▒ s├╝rer; recallÔåÆidentity geri beslemesi bu fazda kapal─▒. Haf─▒za diskte kal─▒r.",
      directive: "FOCUS_RHIZOH"
    };
  }
  if (parsed.verb === "PURGE_CASTLE") {
    ...
    return {
      ...
      reply:
        n > 0
          ? `PURGE tamam: ${n} b├Âlge s─▒f─▒rland─▒ (${owner}). Sim├╝lasyondan kale kald─▒r─▒ld─▒.`
          : `PURGE: ${owner} i├ğin atanm─▒┼ş ki┼şisel b├Âlge yoktu.`
    };
  }
...
class World {
  constructor(maxEntities) {
    ...
    this.CELL_SIZE = 250;
    /** Hayalet / boids i├ğin daha ince h├╝cre (hotspot zincirlerini k─▒salt─▒r). */
    ...
    this.rhizohIdx = -1;
    /** K├Âpr├╝ ├╝zerinden tek ajan i├ğin parabolik transit (tick i├ğinde konum yaz─▒l─▒r). */
    ...
    /** Per-entity shell altitude + swirl phase ÔÇö avoids single shared orbit ring */
  ...
  processGraduationQueue() {
    ...
    uiStore.dispatch({
      type: "ADD_LOG",
      payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: "­şÄô ACADEMY GRADUATION: AGENT -> MASTER" }
    });
  }
    ...
    for (let i = 0; i < this.activeCount; i++) {
      ...
      if (this.state[i] === STATE.RHIZOH) {
        ...
        if (this.xp[i] > 500 && this.petStage[i] === PET_STAGE.SEED) {
          ...
          this.colorDirty[i] = 1;
          uiStore.dispatch({ type: "ADD_LOG", payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: "EVOLUTION: PET Y├£KSELD─░ [BUD]" } });
        }
...
class RealMapCore {
  ...
  /** Fatih beacon ÔÇö zoom / drone targets */
  ...
  static async loadCity(sceneGroup, onLog) {
    ...
    this.isLoading = true;
    try {
      onLog("SATELLITE_LINK: L5 ZONE GENERATION STARTEDÔÇĞ");
      this.buildProceduralFallback(sceneGroup);
      this.isLoaded = true;
      onLog("SATELLITE_LINK: ISTANBUL TILE ┬À procedural REAL_MAP injected.");
    } finally {
...
logBuffer.push({ ts: CODEX_DATE, type: "SYS", data: "RHIZOH L0ÔÇôL13 stack + Sovereign Runtime + Robotics bridge online." });
...
const uiStore = {
  state: {
    viewMode: "CITIZEN",
    realityMode: "GLOBE",
    /** Cesium REAL_MAP y├╝zeyi ÔÇö RealityDirector + gateway faz─▒ ile commit */
    ...
    squadCount: 0,
    eventPulseCount: 0,
    /** Rhizoh 3D sahnesi ├╝zerinde y├╝zen CODEX / medya ankraj─▒ */
    rhizohSceneAnchor: null,
    /** Phase P1 ÔÇö unified shell: world | hall | greenroom | broadcast | studio | profile */
      ...
      if (action.type === "SET_LAYER_FOCUS") {
        ...
        if (prev !== nf) {
          ...
          layerTransitionRing.push({
            ...
            from: prev,
            to: nf,
            label: `${fromSpec?.code ?? "?"}ÔåÆ${toSpec?.code ?? "?"} ┬À ${toSpec?.name ?? ""}`
          });
          logBuffer.push({
            ts: new Date().toLocaleTimeString(),
            type: "SYS",
            data: `LAYER ┬À ${fromSpec?.code ?? "?"} ÔåÆ ${toSpec?.code ?? "?"} (${toSpec?.name ?? ""})`
          });
          next = {
            ...next,
            layerFocus: nf,
...
class ApexEngine {
  constructor(container) {
    ...
    /** Kesilebilir otopilot: OrbitControls ile kullan─▒c─▒ m├╝dahalesi sonras─▒ ~5s programatik hareket yok. */
  ...
  /**
   * ─░ki nokta aras─▒nda kontrol noktas─▒ y├╝kseltilmi┼ş quadratic Bezier ÔÇö d├╝z lerp yerine "swoop".
   */
  ...
  /** L9 b├╝y├╝k olay: ├ğapraz ├ğer├ğeve + bezier yakla┼şma */
  ...
  /** ├çoklu kale k├Âpr├╝leri: parabolik enerji ark─▒ (Line). */
  ...
  focusWorldPoint(x, y, z, distance = 1600, opts = {}) {
    enqueueApexCameraAfterCesiumIfNeeded(() => this._focusWorldPointImpl(x, y, z, distance, opts), "d├╝nya odak");
  }
  ...
  /** Onay sonras─▒ sinematik tur: hedef etraf─▒nda h─▒zl─▒ orbital sweep */
  ...
  /** K─▒sa s├╝reli ÔÇ£sinyal halesiÔÇØ ÔÇö torus mesh, sahneye eklenir ve soluklar */
      ...
      <div className="bg-[#0a0f1d]/95 backdrop-blur-3xl border border-cyan-400/40 p-7 rounded-[2.5rem] flex items-center gap-6 shadow-[0_0_60px_rgba(0,255,255,0.15)] ring-1 ring-white/10 transition-all">
        ...
        <div className="text-left">
          ...
          <div className="text-[9px] font-black text-cyan-400 uppercase tracking-[0.5em]">
            {CODEX_VERSION} ÔÇó {tick}s
          </div>
          <p className="text-[8px] text-white/45 mt-3 normal-case tracking-wide font-semibold leading-snug max-w-[28rem]">
            LLM MMO city simulation (research sandbox) ┬À Playable game (real-time multiplayer feel) ┬À Agent marketplace + user-owned AI ecosystem ┬À Castle Genesis production platform
          </p>
        </div>
      </div>
  ...
  return (
    <div className="rounded-2xl p-3" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      <div className="text-[9px] text-cyan-500/90 tracking-[0.35em] mb-2 font-black flex items-center gap-2">
        <Cpu size={12} /> L0ÔÇôL13 RHIZOH STACK
      </div>
      ...
      <div className="text-[8px] text-white/35 mt-2 font-mono">{LAYER_SPECS.find((x) => x.id === focus)?.name || "ÔÇö"}</div>
    </div>
  );
});
  ...
  return (
    <div className="rounded-2xl p-3 max-h-36 overflow-y-auto no-scrollbar" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      <div className="text-[9px] text-cyan-400 tracking-[0.3em] mb-2 font-black flex items-center gap-2">
        <Layers size={14} /> KATMAN GE├ç─░┼ŞLER─░
      </div>
      {rows.length === 0 ? (
        <div className="text-[8px] text-white/35 normal-case font-semibold">L0ÔÇôL13 d├╝─şmeleri veya komut: LAYER 7 ┬À L12=META ┬À L13=ROBOTICS</div>
      ) : (
        <div className="space-y-1 text-[8px] text-white/65 font-mono">
          {rows.map((r, i) => (
            <div key={`${r.ts}-${i}`} className="border-b border-white/5 pb-1">
              <span className="text-cyan-500">{r.ts}</span> ┬À {r.label}
            </div>
          ))}
        </div>
      )}
...
const LayerExperiencePanel = memo(({ engineRef }) => {
  ...
  const runQuickAction = async (action) => {
    ...
    if (action === "ZOOM CASTLE") {
      ...
      log("LAYER ACTION ┬À Zoom castle");
      return;
    }
    ...
    if (action === "ZOOM AGENT") {
      engine.focusNextAgent();
      log("LAYER ACTION ┬À Zoom agent");
      return;
    }
    log(`LAYER ACTION ┬À Komut terminali: ${action}`);
  };
  ...
  const applyLayerPreset = async (id) => {
    ...
    uiStore.dispatch({
      ...
      payload: {
        ts: new Date().toLocaleTimeString(),
        type: "SYS",
        data: `LAYER PRESET ┬À ${spec.code} (${spec.name}) ┬À ${p.reality}/${p.camera}${satShouldBe ? " + SAT" : ""}`
      }
    });
  };
  ...
  return (
    <div className="rounded-2xl p-4 space-y-3" style={{ background: profile.theme.bg, border: `1px solid ${profile.theme.border}` }}>
      ...
      <div className="text-[8px] text-white/35 leading-relaxed">
        L0-L13 aras─▒nda ge├ği┼şte art─▒k yaln─▒zca log de─şil, katman bazl─▒ g├Ârev, g├Âr├╝n├╝m preset'i ve h─▒zl─▒ operasyon butonlar─▒ uygulan─▒r.
      </div>
    </div>
  );
});
  ...
  return (
    <div className="rounded-2xl p-4 space-y-3" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      <div className="text-[9px] text-sky-300 tracking-[0.35em] flex flex-wrap items-center gap-2 font-black">
        <Camera size={14} /> KAMERA ┬À <span className="text-white">{camMode}</span>
        <span className="text-white/40 font-mono normal-case tracking-normal">{reality === "REAL_MAP" ? "─░stanbul REAL_MAP" : "Globe"}</span>
      </div>
      <div className="flex flex-wrap gap-2">
      ...
      <p className="text-[7px] text-white/40 normal-case tracking-wide leading-relaxed font-semibold">
        Drone: s├╝r├╝kleyerek d├Ân ┬À tekerlek zoom ┬À orta tu┼ş kayd─▒r. Ger├ğek ┼şehir i├ğin ├Ânce ÔÇ£PROCEDURAL CITYÔÇØ (REAL_MAP).
      </p>
    </div>
  );
});
  ...
  return (
    <div className="rounded-[2rem] p-5 backdrop-blur-xl" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <p className="text-[9px] text-white/55 leading-relaxed uppercase tracking-widest mb-3">
        L8 district index 0 ┬À agents with archetypes gain XP ┬À ties to GreenRoom / Spiral training loops.
      </p>
      <div className="text-sm font-mono text-violet-200">
        Tier <span className="text-white">{tier}</span> ┬À Nexus ╬ö <span className="text-emerald-300">{nexus.toFixed(1)}</span>
      </div>
      <div className="text-[8px] text-white/35 mt-2">1500 procedural boxes ┬À {MAX_DISTRICTS} districts ┬À heatmap 256┬▓</div>
    </div>
  );
});
  ...
  return (
    <div className="rounded-2xl p-3" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="text-[9px] text-white/55 space-y-1 max-h-28 overflow-y-auto font-mono no-scrollbar">
        {rows.length === 0 ? <span className="text-white/30">awaiting pulsesÔÇĞ</span> : null}
        {rows.map((ev, i) => (
          <div key={i}>
            {ev.ts} ┬À {ev.eventType} ┬À {ev.eventSource}ÔåÆ{ev.eventTarget} ┬À ╬Á{ev.eventEnergy.toFixed(2)}
          </div>
        ))}
      </div>
    </div>
...
const AgentIdentityLabPanel = memo(({ selectedAgentId, onSelectAgent }) => {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="flex gap-2">
        <button type="button" className={`${btn} border-cyan-400/35 text-cyan-100`} onClick={create}>Kay─▒t</button>
      ...
      <div className="space-y-1 max-h-24 overflow-y-auto no-scrollbar">
        {items.map((a) => (
          ...
          >
            {a.id} ┬À {a.role} ┬À rank {a?.progress?.academyRank ?? 1}
          </button>
        ))}
      </div>
    </div>
      ...
      <div className="space-y-1 max-h-28 overflow-y-auto no-scrollbar">
        {items.map((ev) => (
          <div key={ev.id} className="rounded border border-white/10 bg-black/20 px-2 py-1 text-[8px] normal-case">
            {ev.type} ┬À {ev.topic} ┬À {ev.status}
            {ev.status !== "resolved" ? (
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="text-[8px] text-white/70 normal-case">
        Goals: {(profile?.goals || []).slice(0, 3).join(" ┬À ") || "ÔÇö"}
      </div>
      <div className="text-[8px] text-white/50 normal-case">
        Memory heat: {heat} ┬À E:{context?.episodic?.length || 0} S:{context?.semantic?.length || 0} P:{context?.procedural?.length || 0}
      </div>
...
const StudioMirrorPanel = memo(() => {
  ...
  const refreshMedia = async () => {
    try {
      ...
      setStatus("camera+mic haz─▒r");
    } catch {
      setStatus("media izin hatas─▒");
    }
  };
  ...
  const toggleScreen = async () => {
    ...
    try {
      ...
      setStatus("screen payla┼ş─▒m─▒ a├ğ─▒k");
    } catch {
      setStatus("screen izin hatas─▒");
    }
  };
  ...
  const toggleRecord = () => {
    ...
    if (!s) {
      setStatus("├Ânce media ba┼şlat");
      return;
    }
    if (recording) {
      recRef.current?.stop();
      setRecording(false);
      setStatus("kay─▒t durdu");
      return;
    }
    try {
      ...
      rec.start();
      setRecording(true);
      setStatus("kay─▒t ba┼şlad─▒");
    } catch {
      setStatus("kay─▒t ba┼şlat─▒lamad─▒");
    }
  };
  ...
  const runHealthCheck = async () => {
    ...
    if (!targetToCheck) {
      setStatus("health-check i├ğin target gerekli");
      return;
    }
    try {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="text-[8px] text-white/60 normal-case">
        mic:{micOn ? "on" : "off"} ┬À cam:{camOn ? "on" : "off"} ┬À screen:{screenOn ? "on" : "off"} ┬À rec:{recording ? "on" : "off"} ┬À live:{liveArmed ? "armed" : "idle"}
      </div>
      <div className="flex flex-wrap gap-2">
      ...
      {sessions.length > 0 ? (
        <div className="text-[8px] text-white/65 normal-case max-h-14 overflow-y-auto no-scrollbar">
          {sessions.slice(0, 4).map((s) => (
            <div key={s.id}>{s.protocol} ┬À {s.target || "n/a"} ┬À {s.status}</div>
          ))}
        </div>
      ) : null}
      ...
      <div className="text-[8px] text-white/70 normal-case max-h-24 overflow-y-auto no-scrollbar border border-white/10 rounded p-2 bg-black/20 space-y-1">
        {transcripts.length === 0 ? (
          "Hen├╝z transcript yok."
        ) : (
          transcripts.map((t) => (
            <div key={t.id || `${t.ts}-${t.eventType}`} className="border-b border-white/10 pb-1">
              <div>{new Date(t.ts || Date.now()).toLocaleTimeString()} ┬À {t.eventType} ┬À {t.text}</div>
              <div className="flex gap-2 mt-1">
      ...
      {searchRows.length > 0 ? (
        <div className="text-[8px] text-emerald-200 normal-case max-h-24 overflow-y-auto no-scrollbar border border-emerald-500/30 rounded p-2 bg-emerald-900/10">
          {searchRows.map((t) => `${new Date(t.ts || Date.now()).toLocaleTimeString()} ┬À ${t.eventType} ┬À ${t.text}`).join("\n")}
        </div>
      ) : null}
    </div>
  );
...
const RoboticsMechanicsPanel = memo(({ selectedAgentId }) => {
  ...
  const [message, setMessage] = useState("Hedefe g├╝venli rota planla ve telemetriyi optimize et.");
  const [status, setStatus] = useState("");
  ...
  const [autoTitle, setAutoTitle] = useState("Ak┼şam yeme─şini haz─▒rlamaya ba┼şla");
  ...
  const [commandParams, setCommandParams] = useState('{"text":"Castle update haz─▒r"}');
  ...
  const [broadcastText, setBroadcastText] = useState("Castle teknik geli┼ştirme g├╝ncellemesi yay─▒nda.");
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="flex gap-2">
        <input value={autoTitle} onChange={(e) => setAutoTitle(e.target.value)} placeholder="planner task (├Ârn: ak┼şam yeme─şini haz─▒rla)" className="flex-1 bg-black/35 border border-white/15 rounded px-2 py-1 text-[9px] normal-case" />
      </div>
      <div className="flex gap-2">
      ...
      <div className="text-[8px] text-white/65 normal-case max-h-16 overflow-y-auto no-scrollbar">
        {devices.map((d) => `${d.name} (${d.kind}) ┬À ${d.status} ┬À ${Array.isArray(d.capabilityProfile) ? d.capabilityProfile.join("|") : "-"}`).join("\n")}
      </div>
      <div className="text-[8px] text-white/55 normal-case max-h-16 overflow-y-auto no-scrollbar">
        {telemetryRows.slice(0, 6).map((t) => `${new Date(t.ts || Date.now()).toLocaleTimeString()} ┬À ${t.deviceId} ┬À ${t.mode} ┬À ${t.speed}m/s`).join("\n")}
      </div>
      <div className="text-[8px] text-white/55 normal-case max-h-16 overflow-y-auto no-scrollbar">
        {automations.slice(0, 6).map((a) => `${a.title} ┬À ${a.status} ┬À ${new Date(a.scheduleAt || Date.now()).toLocaleString()}`).join("\n")}
      </div>
      <div className="text-[8px] text-white/55 normal-case max-h-16 overflow-y-auto no-scrollbar">
        {commands.slice(0, 6).map((c) => `${c.deviceId} ┬À ${c.action} ┬À ${c.adapter} ┬À ${c.status}`).join("\n")}
      </div>
      <div className="text-[8px] text-white/55 normal-case max-h-16 overflow-y-auto no-scrollbar">
        {socialChannels.slice(0, 6).map((c) => `${c.platform} ┬À ${c.connectorType} ┬À ${c.status}`).join("\n")}
      </div>
      <div className="text-[8px] text-emerald-200/80 normal-case max-h-16 overflow-y-auto no-scrollbar">
        {ethicsTracks.slice(0, 4).map((t) => `${t.title} ┬À ${t.audience} ┬À ${(t.modules || []).slice(0, 2).join(",")}`).join("\n")}
      </div>
    </div>
  );
});
...
const EventLayerIntelPanel = memo(({ selectedAgentId, selectedLlmConnectionId }) => {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="flex gap-2">
        <input value={waypointName} onChange={(e) => setWaypointName(e.target.value)} placeholder="waypoint ad─▒" className="flex-1 bg-black/35 border border-white/15 rounded px-2 py-1 text-[9px] normal-case" />
      ...
      {waypoints.length > 0 ? (
        <div className="text-[8px] text-white/65 normal-case max-h-12 overflow-y-auto no-scrollbar">
          {waypoints.map((w, i) => (
            <div key={`${w.name}-${i}`}>{i + 1}. {w.name} ┬À {w.persona}</div>
          ))}
        </div>
      ) : null}
      <div className="flex gap-2">
      ...
      <input value={place} onChange={(e) => setPlace(e.target.value)} placeholder="y├╝r├╝y├╝┼ş rotas─▒ / yap─▒ ad─▒" className="w-full bg-black/35 border border-white/15 rounded px-2 py-1 text-[9px] normal-case" />
      <div className="flex gap-2">
        <button type="button" className={`${btn} border-cyan-400/35 text-cyan-100`} onClick={() => void placeBrief()}>Yap─▒ bilgisi</button>
      </div>
      <input value={docUrl} onChange={(e) => setDocUrl(e.target.value)} placeholder="k├╝t├╝phane dok├╝man url (html/txt)" className="w-full bg-black/35 border border-white/15 rounded px-2 py-1 text-[9px] normal-case" />
      <textarea value={docText} onChange={(e) => setDocText(e.target.value)} placeholder="veya metni buraya yap─▒┼şt─▒r (PDF extract)" className="w-full h-16 bg-black/35 border border-white/15 rounded px-2 py-1 text-[9px] normal-case" />
      <div className="flex gap-2 items-center">
        <button type="button" className={`${btn} border-amber-400/35 text-amber-200`} onClick={() => void docBrief()}>Dok├╝man/PDF brief</button>
      ...
      {routeReplies.length > 0 ? (
        <div className="text-[8px] text-white/70 normal-case max-h-24 overflow-y-auto no-scrollbar">
          {routeReplies.map((r, i) => (
            <div key={`route-${i}`}>{i + 1}. {r.name} [{r.persona}] ÔÇö {r.reply}</div>
          ))}
        </div>
      ) : null}
    </div>
...
const RhizohSceneAnchorWindow = memo(() => {
  ...
  const title = String(anchor.title || "Rhizoh ┬À CODEX");
  return (
    <div className="pointer-events-auto fixed bottom-20 right-4 z-[52] w-[min(420px,92vw)] rounded-2xl border border-cyan-400/35 bg-[#030912]/92 shadow-[0_0_40px_rgba(34,211,238,0.12)] backdrop-blur-xl p-3">
      ...
      {!anchor.src && anchor.kind !== "text" ? (
        <div className="text-[8px] text-white/45 normal-case">Bu ankraj i├ğin kaynak URL ba─şl─▒ de─şil; panelden ingest veya replay se├ğin.</div>
      ) : null}
    </div>
  );
});
...
const SovereignCastleCommandPanel = memo(
    ...
    const onDraft = (e) => {
      ...
      uiStore.dispatch({
        ...
        payload: {
          ts: new Date().toLocaleTimeString(),
          type: "SYS",
          data: `L9 ┬À ${d.trigger || "event"} ÔåÆ sosyal kuyruk ┬À ${String(d.text || "").slice(0, 72)}`
        }
      });
  ...
  const connectSocialRow = async (row) => {
    ...
    if (hasServer && socialUnbindLocal[row.id]) {
      ...
      setSocialStatus(`${row.name} ┬À mevcut gateway kanal─▒yla yeniden ba─şland─▒.`);
      return;
    }
    if (!apiBase) {
      setSocialStatus("Gateway yok ÔÇö rhizohLlmHttp.");
      return;
    }
    setSocialStatus(`${row.name} k├Âpr├╝leniyorÔÇĞ`);
    try {
      const res = await fetch(`${apiBase}/social/channels`, {
        method: "POST",
      ...
      const j = await res.json().catch(() => ({}));
      if (j?.ok) {
        setSocialStatus(`${row.name} Sentinel a─ş─▒na kaydedildi.`);
        setSocialUnbindLocal((p) => {
          const n = { ...p };
          delete n[row.id];
          return n;
      ...
      setSocialStatus("Sosyal kanal iste─şi ba┼şar─▒s─▒z.");
    }
  };

  const disconnectSocialRow = (row) => {
    setSocialUnbindLocal((p) => ({ ...p, [row.id]: true }));
    setSocialStatus(`${row.name} UIÔÇÖda ay─▒kland─▒ (sunucudaki etkin kanallar /social/broadcast ile yine tetiklenebilir).`);
  };
  ...
  const sendMeshBroadcast = async () => {
    ...
    const apiBase = getRhizohApiBase();
    if (!apiBase) {
      setSocialStatus("Gateway yok ÔÇö metin yerelde kald─▒.");
      return;
    }
    setSocialStatus("Duyuru g├ÂnderiliyorÔÇĞ");
    try {
      const res = await fetch(`${apiBase}/social/broadcast`, {
        method: "POST",
        headers: getRhizohDevFetchHeaders(),
      ...
      const j = await res.json().catch(() => ({}));
      if (j?.ok) {
        setSocialStatus(`Duyuru kuyru─şa al─▒nd─▒ (${j.queuedChannels ?? "?"} kanal).`);
        setBroadcastDraft("");
        uiStore.dispatch({
          type: "ADD_LOG",
          payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: `SOCIAL BROADCAST ┬À ${text.slice(0, 120)}` }
        });
      } else {
        setSocialStatus(`Broadcast: ${j?.error || "failed"}`);
      }
    } catch {
      setSocialStatus("Broadcast a─ş hatas─▒.");
    }
  };
  ...
  const approveBroadcast = async (b) => {
    ...
    if (attachSimContext && mediaState.playing && mediaState.track) {
      payloadText += `\n\nÔÇö Castle ambiyans: ${String(mediaState.track).slice(0, 96)}`;
    }
    if (attachSimContext && pdfBriefText) {
      const clip = String(pdfBriefText).trim().slice(0, 300);
      payloadText += `\n\nÔÇö CODEX: ${clip}${pdfBriefText.length > 300 ? "ÔÇĞ" : ""}`;
    }
    const apiBase = getRhizohApiBase();
    setSocialStatus("Onayl─▒ yay─▒n iletiliyorÔÇĞ");
    if (apiBase) {
      try {
        const res = await fetch(`${apiBase}/social/broadcast`, {
          method: "POST",
        ...
        const j = await res.json().catch(() => ({}));
        setSocialStatus(j?.ok ? "Yay─▒n gatewayÔÇÖe iletildi." : `Hata: ${j?.error || "broadcast"}`);
      } catch {
        setSocialStatus("Yay─▒n a─ş hatas─▒.");
      }
    } else {
      setSocialStatus("Gateway yok ÔÇö kuyruk ├Â─şesi kald─▒r─▒ld─▒ (demo).");
    }
  ...
  const rejectBroadcast = (id) => {
    ...
    setSocialStatus("Ajan yay─▒n─▒ reddedildi.");
  };
  ...
  const applyPlaylistIndex = useCallback((idx, list) => {
    ...
    const n = rows.length;
    if (!n) {
      setMediaState((s) => ({ ...s, track: "GatewayÔÇÖden ak─▒┼ş yok", src: "", kind: "video", playing: false }));
      return;
    }
    const i = ((idx % n) + n) % n;
    const row = rows[i];
  ...
  const refreshArchive = useCallback(async () => {
    const apiBase = getRhizohApiBase();
    if (!apiBase) {
      setArchiveStatus("API taban─▒ yok (rhizohLlmHttp).");
      setMediaPlaylist([]);
      return;
    }
    setArchiveStatus("SenkronÔÇĞ");
    try {
      ...
      setArchiveStatus(`Kay─▒t: tr ${trJ.items?.length ?? "ÔÇö"} ┬À yay─▒n ${sJ.items?.length ?? "ÔÇö"} ┬À arama ${mJ.items?.length ?? "ÔÇö"}`);
      if (list.length) {
        const row = list[0];
        setMediaIndex(0);
        setMediaState((s) => ({ ...s, track: row.label, src: row.url, kind: row.kind, playing: false }));
      } else {
        setMediaState((s) => ({ ...s, track: "Oynat─▒labilir URL yok (ingest/replay)", src: "", playing: false }));
      }
    } catch {
      setArchiveStatus("Ar┼şiv iste─şi ba┼şar─▒s─▒z.");
    }
  }, []);
  ...
  const runPdfBriefOnRow = useCallback(
    async (row) => {
      ...
      try {
        ...
        setArchiveStatus(j?.ok ? "PDF / dok├╝man ├Âzeti haz─▒r" : `pdf-brief: ${j?.error || "failed"}`);
      } catch {
        ...
        setGarrison((prev) => {
          ...
          const apiRows = drones.map((d) => {
            ...
            return {
              ...
              name: d.name || d.id,
              role: "DroneRelay",
              status: tel ? `${tel.mode || "ÔÇö"} ┬À ${tel.speed ?? "?"} m/s` : String(d.status || "registered")
            };
          });
          return [...core, ...apiRows];
        });
  ...
  const handleSpawn = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        uiStore.dispatch({
          type: "ADD_LOG",
          payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: `KERNEL DSL ┬À ${spawnCmd}` }
        });
        const parsed = parseDSL(spawnCmd);
        const out = parsed ? await applyPersonalCastleDsl(parsed) : { ok: false, reply: "DSL ayr─▒┼şt─▒r─▒lamad─▒." };
        ...
        window.setTimeout(() => {
          ...
          uiStore.dispatch({
            ...
            payload: {
              ts: new Date().toLocaleTimeString(),
              type: "SYS",
              data: "CASTLE ACTIVE ┬À CODEX s├╝reklili─şi ve Event Mesh bu merkeze ba─şland─▒ (L10)."
            }
          });
        }, 400);
      },
  ...
  const handlePurge = async () => {
    ...
    uiStore.dispatch({
      type: "ADD_LOG",
      payload: { ts: new Date().toLocaleTimeString(), type: "WARN", data: `KERNEL DSL ┬À ${purgeCmd}` }
    });
    ...
    setBroadcastDraft("");
    setSocialStatus("");
    setKernelNote("PURGE tamamland─▒; kale sim├╝lasyondan kald─▒r─▒ld─▒.");
  };
  ...
  const mirrorToRhizohScene = () => {
    ...
    if (pdfBriefText) {
      uiStore.dispatch({
        type: "SET_RHIZOH_SCENE_ANCHOR",
        payload: { open: true, title: "CODEX ┬À pdf-brief", kind: "text", text: pdfBriefText }
      });
    }
  };
  ...
  const welcomeName = (rhizohFirstName && String(rhizohFirstName).trim()) || "M─░MAR";
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {castleState === "DORMANT" ? (
            ...
            <span className="text-[8px] text-cyan-200/80 uppercase tracking-widest">SPAWNINGÔÇĞ</span>
          ) : (
      ...
      {castleState === "SPAWNING" ? (
        <div className="text-[9px] text-cyan-300 animate-pulse text-center py-4 tracking-widest uppercase">
          ...
          Synchronizing Spatial EngineÔÇĞ Awakening NodesÔÇĞ
        </div>
      ) : null}
          ...
          <div className="min-h-[120px] max-h-[min(320px,48vh)] overflow-y-auto no-scrollbar pt-2">
            {activeTab === "GREETING" ? (
              <div className="space-y-2 text-[9px] text-white/70 leading-relaxed">
                <div className="text-cyan-200 font-black text-[10px] tracking-widest mb-2">HO┼Ş GELD─░N, {welcomeName}.</div>
                <p>
                  Kaleniz {userLocation?.lat.toFixed(4)}, {userLocation?.lon.toFixed(4)} koordinatlar─▒nda stabilize edildi ({castleType}).
                </p>
                <p>L10 Command Layer ├╝zerinden Event Mesh ve CityMind ak─▒┼şlar─▒ bu merkeze ba─şland─▒.</p>
                <div className="p-2 mt-2 bg-black/40 border border-emerald-500/30 rounded text-emerald-200">
                  Nexus Energy: Optimal. Savunma ve ileti┼şim kanallar─▒ a├ğ─▒k.
                </div>
              </div>
            ) : null}
            ...
            {activeTab === "GARRISON" ? (
              <div className="space-y-2">
                <div className="text-[8px] text-white/40 tracking-widest uppercase mb-2 flex items-center gap-1">
                  <Users size={10} aria-hidden /> Ba─şl─▒ ajanlar &amp; swarm
                </div>
                {garrison.map((a) => (
            ...
            {activeTab === "MODULES" ? (
              <div className="grid grid-cols-2 gap-2">
                  ...
                  onClick={() => {
                    ...
                    uiStore.dispatch({
                      ...
                      payload: {
                        ts: new Date().toLocaleTimeString(),
                        type: "SYS",
                        data: "SWARM RELAY ┬À L13 Robotics ÔÇö gateway /robotics/* telemetri ve komut k├Âpr├╝s├╝"
                      }
                    });
                  }}
                ...
                >
                  ...
                  <div className="text-[7px] text-amber-200/50 mt-1">Drone telemetrisi ┬À detay L13 Robotics panel</div>
                </button>
              </div>
            ) : null}
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 p-3 border border-indigo-400/30 bg-indigo-900/20 rounded">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[8px] text-indigo-300/60">
                      Gateway: transcripts, publish ingest, metadata aramas─▒ ve event-layer/pdf-brief.
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  <input
                    ...
                    placeholder="metadata arama (├Âr. pdf, greenroom)"
                ...
                {mediaPlaylist.length > 0 ? (
                  <div className="max-h-24 overflow-y-auto no-scrollbar space-y-1 rounded border border-white/10 bg-black/25 p-2">
                    {mediaPlaylist.map((row, idx) => (
                      ...
                      >
                        <span className="text-cyan-400/80">{row.kind}</span> ┬À {row.label}
                      </button>
                    ))}
                  </div>
                ) : (
                  <div className="text-[8px] text-white/40 normal-case">Hen├╝z HTTP(S) ingest veya replay URLÔÇÖsi yok; Studio publish veya transcript meta ekleyin.</div>
                )}
                ...
                <div className="p-3 rounded-xl border border-cyan-400/20 bg-black/40">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      onClick={() => {
                        if (!mediaState.src) {
                          setArchiveStatus("├ûnce listeden oynat─▒labilir bir URL se├ğin.");
                          return;
                        }
                ...
                <div className="text-[8px] text-indigo-300/80 tracking-wide uppercase">Arama sonu├ğlar─▒ ┬À dok├╝man ├Âzeti</div>
                <div className="max-h-32 overflow-y-auto no-scrollbar space-y-1">
                  {codexHits.length === 0 ? (
                    <div className="text-[8px] text-white/40 normal-case">Sonu├ğ yok ÔÇö sorguyu de─şi┼ştirip yenileyin.</div>
                  ) : (
                    codexHits.slice(0, 12).map((row, i) => (
                      ...
                      >
                        <div className="font-mono text-cyan-200/80">
                          {row.eventType} ┬À {new Date(row.ts || Date.now()).toLocaleTimeString()}
                        </div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              uiStore.dispatch({
                                type: "SET_RHIZOH_SCENE_ANCHOR",
                                payload: { open: true, title: `Ham metin ┬À ${row.eventType}`, kind: "text", text: t }
                              });
                            }}
            ...
            {activeTab === "NETWORK" ? (
              <div className="space-y-3">
                ...
                <p className="text-[8px] text-white/50 leading-relaxed">
                  D─▒┼ş yay─▒n ve sinyaller i├ğin gateway <span className="text-cyan-200/80">/social/channels</span> ve{" "}
                  <span className="text-cyan-200/80">/social/broadcast</span> u├ğlar─▒na ba─şlan─▒r; Event Mesh (L9) ile birlikte d├╝┼ş├╝n├╝n.
                </p>
                ...
                {SOCIAL_MESH_PLATFORMS.some((p) => meshRowConnected(p)) ? (
                  <div className="mt-1 p-2 border border-cyan-400/20 bg-cyan-900/20 rounded flex items-center gap-2">
                    <input
                      ...
                      placeholder="Ba─şl─▒ kanallara h─▒zl─▒ duyuru (broadcast)ÔÇĞ"
                ...
                <div className="mt-2 p-3 border border-fuchsia-500/30 bg-fuchsia-900/10 rounded-lg">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-3 border-b border-fuchsia-500/20 pb-2">
                    ...
                    >
                      <option value="MANUAL_APPROVAL">S─▒f─▒r otonomi (onay ┼şart)</option>
                      <option value="SEMI_AUTO">Yar─▒ otonom (├Âneriler)</option>
                    </select>
                  </div>
                  <label className="flex items-center gap-2 mb-2 text-[8px] text-white/55 normal-case cursor-pointer">
                    ...
                    />
                    Onayda Library oynat─▒lan ambiyans + CODEX ├Âzetini g├Ânderiye ekle
                  </label>

                  {pendingBroadcasts.length === 0 ? (
                    <div className="text-[8px] text-white/40 text-center py-2 italic normal-case">Onay bekleyen ajan yay─▒n─▒ yok.</div>
                  ) : (
                    <div className="space-y-2 max-h-[min(240px,40vh)] overflow-y-auto no-scrollbar">
                      {pendingBroadcasts.map((broadcast) => (
                        ...
                        >
                          ...
                          {broadcast.trigger ? (
                            <div className="text-[7px] text-fuchsia-300/75 font-mono mb-0.5">L9 ┬À {broadcast.trigger}</div>
                          ) : null}
            ...
            {activeTab === "ALLIANCES" ? (
              <div className="space-y-3">
                ...
                <p className="text-[8px] text-white/50 leading-relaxed normal-case">
                  ├çevrimi├ği kaleler <span className="text-cyan-200/80">active_castles</span> koleksiyonundan okunur; k├Âpr├╝ kurunca Apex sahnesinde
                  parabolik enerji ark─▒ ve (Master / Rhizoh varsa) ajan transit animasyonu ba┼şlar.
                </p>
                {!bridgeRegistryReady ? (
                  <div className="text-[8px] text-amber-200/90 border border-amber-400/30 rounded-lg px-2 py-2 bg-amber-950/20 normal-case">
                    Firebase yap─▒land─▒r─▒lmad─▒ veya oturum yok ÔÇö liste yerel demo modunda; yine de INITIATE BRIDGE sahne ark─▒n─▒ ├╝retebilir.
                  </div>
                ) : null}
                {remoteCastles.length === 0 ? (
                  <div className="text-[8px] text-white/40 text-center py-6 normal-case italic">
                    Aktif uzak kale sinyali yok. Ba┼şka bir mimar kalenizi a├ğ─▒p konum heartbeat g├Ânderdi─şinde burada g├Âr├╝n├╝r.
                  </div>
                ) : (
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-white/45 normal-case mt-0.5">
                            {Number.isFinite(c.lat) && Number.isFinite(c.lon)
                              ...
                              : "ÔÇö"}{" "}
                            ┬À nexus{" "}
                            {typeof c.nexusEnergy === "number" ? `${(c.nexusEnergy * 100).toFixed(0)}%` : "ÔÇö"}
                          </div>
                        </div>
                        <button
                          type="button"
...
const RhizohCommsPanel = memo(
  ...
  const [lastReply, setLastReply] = useState("Hen├╝z yan─▒t yok ÔÇö ilk komutunuzu g├Ânderin.");
  const [isThinking, setIsThinking] = useState(false);
  const thinkWatchdogRef = useRef(0);
  const [source, setSource] = useState("local");
  const [provider, setProvider] = useState("openai");
  /** Gateway: env = sunucu OPENAI_* , user_connection = kay─▒tl─▒ ba─şlant─▒ (giri┼ş + connectionId), auto = ├Ânce sunucu anahtar─▒ */
  ...
  useEffect(() => {
    if (presencePhaseWasRef.current === QPP_PHASE.FADE && presenceFsm.phase === QPP_PHASE.IDLE) {
      setLastReply((r) => (String(r).trim() === "ÔÇö" ? "Rhizoh burada ÔÇö sessiz e┼şlik sona erdi." : r));
    }
      ...
      for (const it of items) {
        ...
        try {
          if (llmKeySource === "user_connection" && !castleAuth?.user) {
            setCommsError("Kay─▒tl─▒ ba─şlant─▒ modu i├ğin hesapla giri┼ş yap─▒n.");
            break;
          }
          let idToken = "";
          try {
          ...
          uiStore.dispatch({
            ...
            payload: {
              ts: new Date().toLocaleTimeString(),
              type: "SYS",
              data: `RHIZOH COMMS (kuyruk) [${out.source}] ┬À ${String(it.message || "").slice(0, 120)}`
            }
          });
        } catch (err) {
          const msg = String(err?.message || err || "─░stek ba┼şar─▒s─▒z");
          setCommsError(msg.length > 160 ? `${msg.slice(0, 160)}ÔÇĞ` : msg);
        } finally {
          setIsThinking(false);
        }
      }
  ...
  const ensureRhizoh = () => {
    ...
    uiStore.dispatch({ type: "ADD_LOG", payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: "RHIZOH ONLINE ┬À avatar materialized." } });
    return idx;
  };
  ...
  const sendPrompt = async (overrideRaw) => {
    ...
    if (gwPh === "offline" || gwPh === "offline_dns") {
      ...
      setLastReply("Ba─şlant─▒ kurulur kurulmaz iletece─şim ÔÇö mesaj s─▒raya al─▒nd─▒.");
      if (overrideRaw == null) setInput("");
      setIsThinking(false);
      uiStore.dispatch({
        type: "ADD_LOG",
        payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: `RHIZOH KUYRUK ┬À ${q.slice(0, 120)}` }
      });
      return;
    }
    setSendPulseNonce((n) => n + 1);
    ...
    thinkWatchdogRef.current = window.setTimeout(() => {
      setIsThinking(false);
      setCommsError((prev) => prev || "Rhizoh yan─▒t─▒ zaman a┼ş─▒m─▒na u─şrad─▒. L├╝tfen tekrar deneyin.");
      dispatchPresence({ type: QPP_EVENT.THINKING_END });
    }, 65_000);
    ensureRhizoh();
    let idToken = "";
    ...
    try {
      ...
      if (origin === "env" || bill === "server")
        setLastBillingLabel("Son kullan─▒m: sunucu anahtar─▒ (ENV)");
      else if (origin === "user_connection" || bill === "user")
        setLastBillingLabel("Son kullan─▒m: hesab─▒n─▒zdaki LLM ba─şlant─▒s─▒");
      else if (origin || bill) setLastBillingLabel(`Son kullan─▒m: ${origin || bill}`);
      ...
      uiStore.dispatch({
        type: "ADD_LOG",
        payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: `RHIZOH COMMS [${out.source}] ┬À ${q}` }
      });
      if (overrideRaw == null) setInput("");
    } catch (err) {
      const msg = String(err?.message || err || "─░stek ba┼şar─▒s─▒z");
      setCommsError(msg.length > 160 ? `${msg.slice(0, 160)}ÔÇĞ` : msg);
      setLastReply("Bu komut g├Ânderilemedi. Ba─şlant─▒n─▒z─▒ kontrol edip yeniden deneyin.");
    } finally {
  ...
  const sendReason = isThinking ? "Rhizoh yan─▒t ├╝retiyorÔÇĞ" : !input.trim() ? "G├Ândermek i├ğin metin yaz─▒n" : "";
  ...
  const inputPlaceholder =
    inQuietPresence
      ...
      : isThinking
        ? "D├╝┼ş├╝nceni s├╝rd├╝r."
        : "Katman, kamera veya g├Ârev yaz─▒n; gateway a├ğ─▒ksa sunucu LLM, yoksa yerel yan─▒t.";
    ...
    >
      ...
      {gatewayModel ? (
        <div className="rounded-xl border border-white/10 bg-black/25 px-2 py-2 text-[9px] text-white/75 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-semibold text-cyan-100/95">A─ş ge├ğidi:</span> {gatewayModel.headline}
      ...
      <div className="text-[8px] text-white/45">
        Yan─▒t kayna─ş─▒: <span className="text-white/75">{source}</span>
        {isThinking ? " ┬À ├╝retiliyorÔÇĞ" : ""}
      </div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          aria-label="LLM sa─şlay─▒c─▒ se├ğimi"
        ...
        >
          <option value="openai">OpenAI (gateway / sunucu anahtar─▒)</option>
          <option value="anthropic">Anthropic</option>
          <option value="gemini">Google Gemini</option>
          <option value="xai">xAI</option>
          <option value="deepseek">DeepSeek</option>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          aria-label="LLM anahtar kayna─ş─▒"
          className="bg-black/35 border border-white/15 rounded-lg px-2 py-1 text-[9px] text-white normal-case tracking-normal outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
        >
          <option value="auto">Otomatik (├Ânce sunucu ENV, yoksa ba─şlant─▒m)</option>
          <option value="env">Yaln─▒z sunucu (ENV)</option>
          <option value="user_connection">Yaln─▒z kay─▒tl─▒ ba─şlant─▒m (giri┼ş gerekir)</option>
        </select>
      </div>
      {lastBillingLabel ? (
      ...
      <p className="text-[8px] text-white/50 leading-relaxed">
        Ba─şlant─▒ kimli─şi sunucuda yaln─▒zca sizin hesab─▒n─▒z─▒n alt koleksiyonundan ├ğ├Âz├╝l├╝r; anahtar taray─▒c─▒ya g├Ânderilmez. Net faturalama i├ğin ┬½Yaln─▒z sunucu┬╗ veya ┬½Yaln─▒z ba─şlant─▒m┬╗ se├ğin.
      </p>
      {commsError ? (
      ...
      <div className="text-[8px] text-white/75 leading-relaxed min-h-[2.5rem] border border-white/5 rounded-lg px-2 py-1.5 bg-black/20">
        {inQuietPresence ? (
          <span className="text-cyan-100/70 italic">
            Quiet presence ÔÇö Rhizoh holds space{lastReply && lastReply !== "ÔÇö" ? `: ${lastReply}` : ""}.
          </span>
        ) : (
          lastReply
        )}
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            aria-label="Rhizoh mesaj─▒n─▒ g├Ânder"
            title={sendReason || "G├Ânder"}
          >
            {isThinking ? "ÔÇĞ" : "G├Ânder"}
          </button>
        </div>
        <p id="rhizoh-comms-send-hint" className="text-[8px] text-white/45">
          {sendReason || "Enter ile de g├Ânderebilirsiniz."}
        </p>
...
const AutonomousCompanyDebugPanel = memo(({ runtimeRef }) => {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.bg, border: `1px solid ${theme.border}` }}>
      ...
      <div className="text-[8px] text-white/70 normal-case">
        contracts:{snap?.contractCount ?? 0} ┬À approvals:{snap?.approvalPending ?? 0} ┬À tasks(done/failed):
        {snap?.taskCounts?.completed ?? 0}/{snap?.taskCounts?.failed ?? 0}
      </div>
      <div className="text-[8px] text-white/60 normal-case">
        kill: {snap?.killState?.active ? `${snap.killState.level} ┬À ${snap.killState.reason}` : "inactive"} ┬À
        events:{snap?.eventCount ?? 0} ┬À latest:{snap?.latestEventType ?? "none"}
      </div>
      ...
      <div className="text-[8px] text-white/60 normal-case max-h-14 overflow-y-auto no-scrollbar">
        {(events ?? []).map((e) => (
          <div key={e.eventId}>[{e.frameId}] {e.type} ┬À {e.traceId ?? "-"}</div>
        ))}
      </div>
      <div className="text-[8px] text-white/45 normal-case">
        replay(trace): {traceReplay?.traceId ?? "none"} ┬À events:{traceReplay?.eventCount ?? 0} ┬À
      ...
      <div className="text-[8px] text-white/60 normal-case max-h-12 overflow-y-auto no-scrollbar">
        {(snap?.recentTasks ?? []).map((t) => (
          <div key={t.taskId}>{t.agentId.replace("RHIZOH_", "").replace("_AGENT", "")}: {t.status} ┬À {t.traceId}</div>
        ))}
      </div>
    </div>
  );
...
export default function AppRhizoh528() {
  ...
  const commandPanelAux = useSyncExternalStore(
    subscribeRhizohCommandPanelAuxExpandedV0,
    getRhizohCommandPanelAuxExpandedSnapshotV0,
    () => false
  );
  const toggleCommandPanelAux = useCallback(() => {
    writeRhizohCommandPanelAuxExpandedV0(!getRhizohCommandPanelAuxExpandedSnapshotV0());
  }, []);
  ...
  /** Drawer studio: WORLD (map) ┬À LIVE (director) ┬À KERNEL (console). */
  ...
  /** Primitives only ÔÇö object snapshots from useSyncExternalStore must be referentially stable when inputs unchanged. */
  ...
  useEffect(() => {
    ...
    // Stabilization: seed write zaten continuity sync callback'ini tetikler; burada ek tick atmayal─▒m.
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      if (prev && prev !== phase) {
        ...
        // Stabilization: closure milestone write continuity sync ├╝zerinden zaten UI g├╝ncelliyor.
      }
  ...
  const voiceEngineV3Ref = useRef(null);
  const voiceSttMaxRecordTimerRef = useRef(0);
  const voiceEngineV3BridgeRef = useRef(null);
  /** S├╝rekli dinleme: LLM/TTS s─▒ras─▒nda rec.onend ile erken mic a├ğ─▒lmas─▒n─▒ erteler. */
  ...
  /** Chrome STT: final transcript sometimes arrives only on onend ÔÇö buffer until flush. */
  ...
  const voiceLoopStartInFlightRef = useRef(false);
  const voiceSttSessionStartedAtRef = useRef(0);
  const voiceSttHadInterimRef = useRef(false);
  const voiceSttLastSessionErrorRef = useRef("");
  const voiceSttNoSpeechRetryRef = useRef(0);
  const voiceSttNoSpeechRetryTimerRef = useRef(0);
  const lastVoiceSilentStopWarnAtRef = useRef(0);
      ...
      if (n > 0) {
        uiStore.dispatch({
          ...
          payload: {
            ts: new Date().toLocaleTimeString(),
            type: "SYS",
            data: `L9 EXECUTION GATE ┬À ${n} ertelenmi┼ş taslak y├╝zey aktif olunca yay─▒nland─▒`
          }
        });
      }
    }
  ...
  const welcomeCard = useMemo(() => {
    ...
    return {
      ...
      secondary: `${memoryNarrative} ┬À ${entityCount} pulses in the field.`,
  ...
  useEffect(() => {
    ...
    window.__rhizoh.debug = () => ({
      ...
      voiceAdapter: window.__rhizoh?.voiceAdapter ?? null
    });
    window.__rhizoh.metrics = () => {
      const rows = [
    ...
    window.__rhizoh.policy = () => rhizohConversationUx.decisionOverlay;
    ...
    return () => {
      try {
        ...
        delete window.__rhizoh.debug;
        delete window.__rhizoh.metrics;
        delete window.__rhizoh.policy;
        delete window.__rhizoh.truth;
      } catch {
        /* noop */
      }
    };
    ...
    async (peer) => {
      ...
      uiStore.dispatch({
        ...
        payload: {
          ts: new Date().toLocaleTimeString(),
          type: "SYS",
          data: `RHIZOH-MIRROR ┬À INITIATE BRIDGE ÔåÆ ${String(peer.id).slice(0, 10)}ÔÇĞ (${ok ? "registry" : "local-only"})`
        }
      });
      try {
  ...
  /** Product routes → shell + layer/reality + domain gate (React Router; refresh-safe). */
  useEffect(() => {
    const pathname = location.pathname;
    const search = location.search;
    const gateUserId = castleAuth?.user?.uid || null;
    ...
    if (liveMatch?.params?.traceId) {
      ...
      signalCastlePresenceV0("broadcast_live", { traceId: tid });
      return;
    }
    ...
    if (greenSlugMatch?.params?.roomUid) {
      ...
      void setRealityMode("GLOBE", { source: "ROUTE_GREENROOM" });
      const slug = decodeURIComponent(String(greenSlugMatch.params.roomUid));
      if (slug === "main") {
        ensureGreenRoomMainHallBound();
      }
      runDomainGateForPathV0(pathname, { userId: gateUserId });
      signalCastlePresenceV0("greenroom_enter", { roomUid: slug });
      return;
    }
    ...
    if (matchPath({ path: "/broadcast/:broadcastUid", end: true }, pathname)) {
      uiStore.dispatch({ type: "SET_PRODUCT_SURFACE", payload: "broadcast" });
      runDomainGateForPathV0(pathname, { userId: gateUserId });
      signalCastlePresenceV0("broadcast_enter", {
        broadcastUid: decodeURIComponent(String(matchPath({ path: "/broadcast/:broadcastUid", end: true }, pathname)?.params?.broadcastUid || "main"))
      });
      return;
    }
    ...
    if (matchPath({ path: "/hall/:roomUid", end: true }, pathname)) {
      ...
      uiStore.dispatch({ type: "SET_LAYER_FOCUS", payload: 10 });
      void setRealityMode("GLOBE", { source: "ROUTE_HALL" });
      runDomainGateForPathV0(pathname, { userId: gateUserId });
      signalCastlePresenceV0("hall_enter", {
        roomUid: decodeURIComponent(String(matchPath({ path: "/hall/:roomUid", end: true }, pathname)?.params?.roomUid || "main"))
      });
      return;
    }

    if (matchPath({ path: "/map", end: true }, pathname)) {
      navigate("/world/space", { replace: true });
      return;
    }
    ...
    if (matchPath({ path: "/spiral", end: true }, pathname)) {
      ...
      void setRealityMode("GLOBE", { source: "ROUTE_SPIRAL" });
      runDomainGateForPathV0(pathname, { userId: gateUserId });
      return;
    }

    if (matchPath({ path: "/settings", end: true }, pathname) || matchPath({ path: "/observer/settings", end: true }, pathname)) {
      uiStore.dispatch({ type: "SET_PRODUCT_SURFACE", payload: "profile" });
      setDrawerStudioTab("chat");
      runDomainGateForPathV0(pathname, { userId: gateUserId });
      return;
    ...
    const worldFromPath = resolveWorldDomainFromPathV0(pathname);
    if (worldFromPath) {
      writeRhizohWorldDrawerDomainV0(worldFromPath);
      uiStore.dispatch({ type: "SET_PRODUCT_SURFACE", payload: "world" });
      closeAllRhizohProductSurfacePanelsV0();
      runDomainGateForPathV0(pathname, { userId: gateUserId });
      void setRealityMode("REAL_MAP", { source: "ROUTE_WORLD_DOMAIN", productSurface: "world" });
      if (worldFromPath === RHIZOH_WORLD_DRAWER_DOMAIN_V0.SPACE && readRhizohWorldMapToolV0() === "globe") {
  ...
  useEffect(() => {
    ...
    if (s === "world") {
      if (isRhizohWorldDomainPathV0(location.pathname)) {
        void setRealityMode("REAL_MAP", { source: "PRODUCT_SHELL_WORLD_DOMAIN", productSurface: "world" });
        uiStore.dispatch({ type: "SET_LAYER_FOCUS", payload: 10 });
    ...
    if (s === "hall") {
      void setRealityMode("GLOBE", { source: "PRODUCT_SHELL_HALL" });
      uiStore.dispatch({ type: "SET_LAYER_FOCUS", payload: 10 });
      uiStore.dispatch({ type: "SET_GREENROOM", payload: false });
    ...
    if (s === "greenroom") {
      void setRealityMode("GLOBE", { source: "PRODUCT_SHELL_GREENROOM" });
      uiStore.dispatch({ type: "SET_LAYER_FOCUS", payload: 5 });
    ...
    if (s === "broadcast") {
      void setRealityMode("GLOBE", { source: "PRODUCT_SHELL_BROADCAST" });
    ...
    if (s === "studio") {
      void setRealityMode("GLOBE", { source: "PRODUCT_SHELL_STUDIO" });
    ...
    if (s === "profile") {
      void setRealityMode("GLOBE", { source: "PRODUCT_SHELL_PROFILE" });
      uiStore.dispatch({ type: "SET_LAYER_FOCUS", payload: 11 });
      uiStore.dispatch({ type: "SET_GREENROOM", payload: false });
  ...
  const onApplyWorldMapToolV0 = useCallback(
    (mapTool, source = "MAP_TOOL_EXPLICIT") => {
      if (!isRhizohWorldDomainPathV0(location.pathname)) {
        navigate(resolveWorldDomainPathV0("space"));
      }
      closeAllRhizohProductSurfacePanelsV0();
      uiStore.dispatch({ type: "SET_PRODUCT_SURFACE", payload: "world" });
    ...
    [remoteCastles, location.pathname, navigate]
  );
  ...
  const onShareWorldInviteV0 = useCallback(async () => {
    const out = await shareWorldInviteV0({
      reviewerId: castleAuth?.user?.uid || "friend",
      label: castleAuth?.user?.displayName || "Friend"
    });
    pushT0ContinuityPulseV0(
      uiLocaleV0 === "tr" ? out.messageTr : out.messageEn,
      "share_invite"
    );
    dispatchDomainCapabilityV0(RHIZOH_DOMAIN_ID_V0.WORLD, RHIZOH_DOMAIN_CAPABILITY_V0.SOCIAL, {
      action: "share_invite",
      url: out.url
    });
  }, [castleAuth?.user?.uid, castleAuth?.user?.displayName, uiLocaleV0]);

  const onWorldModeSelectV0 = useCallback(
    (modeId) => {
      dispatchDomainCapabilityV0(RHIZOH_DOMAIN_ID_V0.WORLD, RHIZOH_DOMAIN_CAPABILITY_V0.TENSOR, {
        intent: `mode:${modeId}`
      });
      if (modeId === RHIZOH_WORLD_SYSTEM_MODE_V0.SPIRAL) {
        navigate("/spiral");
  ...
  const experienceSessionCtxV0 = useMemo(
    () => loadRhizohExperienceSessionContextV0() || createInitialRhizohExperienceSessionContextV0(),
    []
  );
  ...
  const finishVoiceTurnIfNeeded = useCallback(() => {
    ...
    stampVoiceUserGestureV0("after_tts_refresh");
  ...
  const handleVoiceTranscript = useCallback(
    async (
      ...
      /** @type {ReturnType<typeof evaluateVoiceCommitmentFromBandV0> | null} */
      let pipelinePreCommitment = null;

      const isChromeLike =
        voiceSource === "barge_in" || voiceSource === "mic" || voiceSource === "mic_onend";
      if (isChromeLike) {
        const pipe = runVoiceTranscriptWitnessPipelineV0({
          ...
          confidence: Number.isFinite(Number(confidence))
            ...
            : voiceSource === "barge_in"
              ? 0.5
              : undefined,
          strategy: strategy || (voiceSource === "barge_in" ? "browser_stt" : ""),
          source: voiceSource,
      ...
      if (isVoiceDispatch) {
        ...
        const pre =
          ...
          evaluateVoiceCommitmentFromBandV0(
            witnessed?.observation?.band || VOICE_DIRECTED_SPEECH_BAND.UNKNOWN,
            { source: voiceSource }
          );
        if (!shouldDispatchVoiceToLlmV0(pre, { sanityAccepted: true })) {
          ...
          publishVoiceBehavioralCommitmentV0(observeOnly, {
            stage: "dispatch",
            phase: "llm_blocked"
          });
          ...
          recordVoiceInfluenceAttributionV0(
            deriveVoiceInfluenceAttributionV0({
              band: witnessed?.observation?.band || pre.band,
              source: voiceSource,
              stage: "dispatch_llm_blocked",
              commitment: observeOnly,
              sanityAccepted: true,
              turnAccepted: false
            }),
            { preview: trimmed.slice(0, 96), phase: "llm_blocked" }
          );
          if (manageVoiceTurn) finishVoiceTurnIfNeeded();
          return;
        }
      ...
      try {
        ...
        logCastleLifecycleV0("voice_turn_llm", {
        ...
        recordVoiceInfluenceAttributionV0(
          ...
          {
            preview: String(procV.uiReply || "").slice(0, 96),
            traceId: out.traceId || "",
            replySource: String(out.source || "voice")
          }
        );
        if (!procV.skipSpeech) {
          await speakAfterVoiceInstantAckSmoothV0(() => {
        ...
        uiStore.dispatch({
          type: "ADD_LOG",
          payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: `VOICE┬ÀRHIZOH ┬À ${trimmed.slice(0, 120)}` }
        });
      } catch (err) {
        ...
        const msg =
          String(err?.message || err).includes("timeout") || String(err?.name || "") === "AbortError"
            ? "Yan─▒t ├ğok uzun s├╝rd├╝; k─▒sa bir c├╝mleyle tekrar dene."
            : "Ses ile Rhizoh hatt─▒nda hata olu┼ştu; yazarak da g├Ânderebilirsin.";
        if (manageVoiceTurn) {
  ...
  const ensureAmbientSound = useCallback(async () => {
    if (ambientCtxRef.current) {
      return resumeVoiceAudioContextV0(ambientCtxRef.current);
    }
    const AudioContextCtor = window.AudioContext || window.webkitAudioContext;
    if (!AudioContextCtor) return "none";
    ...
    return resumeVoiceAudioContextV0(ctx);
  }, []);

  const maybeWarnVoiceSilentStopV0 = useCallback(
    (promptKey) => {
      const now = Date.now();
      if (now - Number(lastVoiceSilentStopWarnAtRef.current || 0) < 12000) return;
      lastVoiceSilentStopWarnAtRef.current = now;
      speakRhizoh(voiceSttEmptyPromptTrV0(promptKey));
    },
    [speakRhizoh]
  );

  const requestVoiceMicRestartV0 = useCallback(
    (keepAlive, context = "turn_end", opts = {}) => {
      if (!keepAlive && context !== "after_tts") return;
      ...
      const lastSessionHadResult =
        ...
        context === "after_tts" ||
        voiceSttGotAnyResultRef.current === true;

      const decision = canRequestVoiceSttAutoRestartV0({
        context,
        lastSessionHadResult,
        keepAlive
      });
      if (!decision.ok) {
        if (decision.reason === "silent_abort_cooldown") {
          logVoiceWarnV0("STT_RESTART_COOLDOWN", {
            context,
            remainingMs: decision.remainingMs
          });
        } else if (decision.reason === "onresult_empty_no_prior_result") {
          logVoiceWarnV0("STT_RESTART_BLOCKED_NO_PRIOR_RESULT", { context });
        } else if (decision.reason === "gesture_expired") {
          logVoiceInfoV0("STT_RESTART_GESTURE_SOFT", { context, reason: decision.reason });
        } else if (decision.reason === "onend_silent_abort" || decision.reason === "context_not_stt") {
          logVoiceWarnV0("STT_RESTART_BLOCKED", { context, reason: decision.reason });
        }
        setRhizohFieldState("IDLE");
        return;
      }

      if (decision.softDegrade) {
        logVoiceInfoV0("STT_RESTART_SOFT_DEGRADE", {
          context,
          reason: decision.reason
        });
      }

      const pre = probeVoiceSttStartPreconditionsV0(ambientCtxRef.current, {
        softGesture: keepAlive && (context === "after_tts" || decision.softDegrade === true)
      });
      if (!pre.ok) {
        if (pre.reason === "gesture_expired" && keepAlive && context === "after_tts") {
          logVoiceInfoV0("STT_RESTART_GESTURE_SOFT", { context, reason: pre.reason });
        } else {
          logVoiceWarnV0("STT_RESTART_BLOCKED", { context, ...pre });
          maybeWarnVoiceSilentStopV0(
            pre.reason === "tab_hidden" ? "tab" : pre.reason === "gesture_expired" ? "gesture_rebind" : "audio"
          );
          setRhizohFieldState("IDLE");
          return;
        }
      } else if (pre.softDegrade) {
        logVoiceInfoV0("STT_RESTART_SOFT_DEGRADE", { context, reason: pre.reason });
      }

      const advisory = getVoiceGestureAdvisoryV0();
      if (advisory.hintKey === "gesture_soft_rebind") {
        logVoiceInfoV0("STT_RESTART_ADVISORY", {
          context,
          advisoryLevel: advisory.level,
          ageMs: advisory.ageMs
        });
      }
      ...
      window.setTimeout(() => {
        if (!voiceLoopEnabledRef.current) return;
        if (voiceSttStartInFlightRef.current) return;
        const decisionLater = canRequestVoiceSttAutoRestartV0({
          context,
          lastSessionHadResult,
          keepAlive
        });
        if (!decisionLater.ok && !(keepAlive && context === "after_tts" && decisionLater.reason === "gesture_expired")) {
          logVoiceWarnV0("STT_RESTART_DEFERRED", {
            context: `${context}_delayed`,
            reason: decisionLater.reason
          });
          setRhizohFieldState("IDLE");
          return;
        }
  ...
  const scheduleVoiceMicRestartV0 = useCallback(
    (keepAlive, opts = {}) => {
      requestVoiceMicRestartV0(keepAlive, opts.context || "onresult_empty", opts);
    },
    [requestVoiceMicRestartV0]
  );
  ...
  const startVoiceToRhizoh = useCallback(
    async (keepAlive = false, opts = {}) => {
      const userGestureUrgent = opts?.userGestureUrgent === true;
      installVoiceSttTelemetryV0();
      logVoiceInfoV0("VOICE_INIT_STATE", { ...getVoiceInitSnapshotV0(), userGestureUrgent });

      if (voiceSttStartInFlightRef.current) {
        const engineBusy = voiceEngineV3Ref.current?.isBusy?.();
        if (!micListening && !engineBusy && rhizohFieldState === "IDLE") {
          voiceSttStartInFlightRef.current = false;
          logVoiceWarnV0("STT_INFLIGHT_STALE_RESET", {});
        } else {
          logVoiceInfoV0("STT_START_SKIPPED", { reason: "in_flight" });
          return;
        }
      }
      voiceSttStartInFlightRef.current = true;

      let adapterSnap = getVoiceAdapterRegistrySnapshot();
      if (!adapterSnap.hydrated) {
        adapterSnap = await awaitVoiceAdapterRegistryReadyV0();
      }
      if (!getRhizohVoiceInputAdapterV0() || adapterSnap.fallbackMode) {
        voiceSttStartInFlightRef.current = false;
      ...
      if (isVoiceEngineV3EnabledV0()) {
        ...
        if (!preStartV3.ok) {
          voiceSttStartInFlightRef.current = false;
          logVoiceWarnV0("V3_PRECONDITION", { ...preStartV3 });
          setMicListening(false);
          setRhizohFieldState("IDLE");
          maybeWarnVoiceSilentStopV0(
            preStartV3.reason === "tab_hidden"
              ? "tab"
              : preStartV3.reason === "gesture_expired"
                ? "gesture_rebind"
                : "audio"
          );
          return;
        }
        if (preStartV3.softDegrade) {
          logVoiceInfoV0("V3_GESTURE_SOFT_DEGRADE", {
            reason: preStartV3.reason,
        ...
        if (!voiceEngineV3BridgeRef.current) {
          voiceEngineV3BridgeRef.current = createVoiceEngineV3TurnBridgeV0({
            refs: {
              voiceEngineV3: voiceEngineV3Ref,
              voiceSttStartInFlight: voiceSttStartInFlightRef,
              voiceSttMaxRecordTimer: voiceSttMaxRecordTimerRef,
              voiceSttGotAnyResult: voiceSttGotAnyResultRef
            },
            callbacks: {
              setRhizohFieldState,
              setMicListening,
        ...
        if (
          voiceEngineV3Ref.current &&
          engineState &&
          engineState !== "IDLE" &&
          engineState !== "RECORDING"
        ) {
          voiceSttStartInFlightRef.current = false;
          logVoiceWarnV0("V3_START_BLOCKED", { state: engineState });
          return;
        }
        if (voiceEngineV3Ref.current?.isBusy?.()) {
          voiceSttStartInFlightRef.current = false;
      ...
      const preStart = probeVoiceSttStartPreconditionsV0(ambientCtxRef.current);
      if (!preStart.ok) {
        voiceSttStartInFlightRef.current = false;
        logVoiceWarnV0("STT_PRECONDITION", { ...preStart });
        setMicListening(false);
        setRhizohFieldState("IDLE");
        maybeWarnVoiceSilentStopV0(
          preStart.reason === "tab_hidden"
            ? "tab"
            : preStart.reason === "gesture_expired"
              ? "gesture_rebind"
              : "audio"
        );
        return;
      ...
      voicePendingTranscriptRef.current = "";
      voiceSttDispatchedRef.current = false;
      voiceSttGotAnyResultRef.current = false;
      voiceSttHadInterimRef.current = false;
      voiceSttLastSessionErrorRef.current = "";
      if (!opts?.langAttempt) voiceSttNoSpeechRetryRef.current = 0;
      voiceSttSessionStartedAtRef.current = Date.now();
      const rec = new Ctor();
      rec.lang = resolveVoiceSttLangV0({ langAttempt: opts?.langAttempt });
      rec.interimResults = true;
      rec.maxAlternatives = 1;
      rec.continuous = resolveVoiceSttContinuousV0(keepAlive);
      rec.__castleStopRequested = false;
      noteVoiceSttEventV0("STT_SESSION_BEGIN", {
        lang: rec.lang,
        continuous: rec.continuous,
        keepAlive,
        adapter: adapterSnap.sttStatus
      });
      logVoiceWarnV0("STT_SESSION_BEGIN", {
        lang: rec.lang,
        continuous: rec.continuous,
        keepAlive,
        adapter: adapterSnap.sttStatus,
      ...
      rec.onaudiostart = () => {
        noteVoiceSttEventV0("STT_AUDIO_START", {});
        logVoiceInfoV0("STT_AUDIO_START", {});
      };
      rec.onsoundstart = () => {
        noteVoiceSttEventV0("STT_SOUND_START", {});
        logVoiceInfoV0("STT_SOUND_START", {});
      };
      rec.onsoundend = () => {
        noteVoiceSttEventV0("STT_SOUND_END", {});
        logVoiceInfoV0("STT_SOUND_END", {});
      };
      rec.onnomatch = () => {
        noteVoiceSttEventV0("STT_NOMATCH", {});
        logVoiceWarnV0("STT_NOMATCH", { lang: rec.lang });
      };
      rec.onresult = async (ev) => {
        try {
          ...
          if (text) {
            voicePendingTranscriptRef.current = text;
            voiceSttHadInterimRef.current = true;
          }
          noteVoiceSttEventV0("STT_RESULT", {
            isFinal,
            chars: text.length,
            preview: text.slice(0, 48)
          });
          logVoiceInfoV0("STT_RESULT", {
            isFinal,
            chars: text.length,
            preview: text.slice(0, 48)
          });
          ...
          if (!text) {
            if (keepAlive && !voiceTurnBusyRef.current) {
              scheduleVoiceMicRestartV0(keepAlive, {
          ...
          voiceSttNoSpeechRetryRef.current = 0;
          rec.__castleStopRequested = true;
          try {
            rec.stop();
          } catch {
            /* noop */
          }
          voiceSttDispatchedRef.current = true;
          noteVoiceSttEventV0("STT_FINAL", { chars: text.length, preview: text.slice(0, 96) });
          logVoiceInfoV0("STT_FINAL", { chars: text.length, preview: text.slice(0, 96) });
          ...
          voiceTurnBusySinceRef.current = 0;
          setVoiceLoopEnabled(false);
          speakRhizoh("Ses ile Rhizoh hatt─▒nda hata olu┼ştu; yazarak g├Ândermeyi dene.");
        }
      };
      rec.onend = () => {
        voiceSttStartInFlightRef.current = false;
        ...
        const pending = String(voicePendingTranscriptRef.current || "").trim();
        if (pending && !voiceSttDispatchedRef.current && !voiceTurnBusyRef.current) {
          voiceSttDispatchedRef.current = true;
          logVoiceInfoV0("STT_ONEND_FLUSH", { chars: pending.length, preview: pending.slice(0, 96) });
          void handleVoiceTranscript(pending, { manageVoiceTurn: keepAlive, source: "mic_onend" });
          return;
        }
        ...
        const empty = classifyEmptySttOnEndV0({
          pending,
          gotAnyResult: voiceSttGotAnyResultRef.current,
          keepAlive,
          visibility: document.visibilityState,
          audioState: ambientCtxRef.current?.state,
          sessionDurationMs: Date.now() - Number(voiceSttSessionStartedAtRef.current || Date.now()),
          gestureAgeMs: getVoiceGestureTrustV0().ageMs,
          hadInterimActivity: voiceSttHadInterimRef.current,
          lastSttError: voiceSttLastSessionErrorRef.current,
          gatewayDegraded: ["offline", "offline_dns", "reconnecting", "connecting"].includes(
            String(gatewayUx?.phase || "").toLowerCase()
          )
        });
        if (!voiceSttDispatchedRef.current) {
          noteVoiceSttEventV0("STT_ONEND_EMPTY", {
            reason: empty.reason,
            confidence: empty.silentEnd?.confidence,
            gotAnyResult: voiceSttGotAnyResultRef.current
          });
          logVoiceInfoV0("STT_ONEND_EMPTY", {
            reason: empty.reason,
            kind: empty.kind,
            gotAnyResult: voiceSttGotAnyResultRef.current,
            silentEnd: empty.silentEnd
          });
        }
        if (empty.blockAutoRestart) {
          voiceAutoRestartBlockedRef.current = true;
        }
        if (empty.applySilentAbortCooldown && empty.cooldownMs > 0 && voiceSttLastSessionErrorRef.current !== "no-speech") {
          blockVoiceSttAutoRestartV0(empty.cooldownMs);
          logVoiceWarnV0("STT_SILENT_ABORT_COOLDOWN", {
            cooldownMs: empty.cooldownMs,
            reason: empty.reason,
            confidence: empty.silentEnd?.confidence,
            score: empty.silentEnd?.score
          });
        }
        if (empty.shouldSoftRestart && keepAlive && !voiceTurnBusyRef.current) {
          logVoiceInfoV0("STT_ONEND_SOFT_RESTART", {
            confidence: empty.silentEnd?.confidence,
            score: empty.silentEnd?.score
          });
          scheduleVoiceMicRestartV0(keepAlive, {
            context: "onend_ambiguous",
            lastSessionHadResult: voiceSttHadInterimRef.current
          });
          setRhizohFieldState("IDLE");
          return;
        }
        if (empty.shouldPrompt) {
          maybeWarnVoiceSilentStopV0(empty.promptKey || "gesture_rebind");
          setRhizohFieldState("IDLE");
          setRhizohInlineError({
            title: "Mikrofon bekliyor",
            detail: voiceSttEmptyPromptTrV0(empty.promptKey || "gesture_rebind")
          });
          return;
        }
        setRhizohFieldState("IDLE");
      ...
      rec.onerror = (e) => {
        ...
        if (benign) {
          voiceSttLastSessionErrorRef.current = err;
          noteVoiceSttEventV0("STT_ERROR_BENIGN", { code: err, message: msg });
          logVoiceInfoV0("STT_ERROR_BENIGN", { code: err, message: msg });
          if (err === "no-speech" && keepAlive && voiceLoopEnabledRef.current && !voiceTurnBusyRef.current) {
            const plan = planVoiceSttNoSpeechRetryV0({
              retryCount: voiceSttNoSpeechRetryRef.current,
              langAttempt: opts?.langAttempt,
              keepAlive
            });
            if (plan.ok) {
              voiceSttNoSpeechRetryRef.current = plan.nextRetryCount;
              if (voiceSttNoSpeechRetryTimerRef.current) {
                window.clearTimeout(voiceSttNoSpeechRetryTimerRef.current);
              }
              logVoiceInfoV0("STT_NO_SPEECH_RETRY", {
                attempt: plan.nextRetryCount,
                delayMs: plan.delayMs,
                lang: resolveVoiceSttLangV0({ langAttempt: plan.langAttempt }),
                primary: VOICE_STT_LANG_PRIMARY,
                fallback: VOICE_STT_LANG_FALLBACK
              });
              voiceSttNoSpeechRetryTimerRef.current = window.setTimeout(() => {
                voiceSttNoSpeechRetryTimerRef.current = 0;
                if (!voiceLoopEnabledRef.current || voiceTurnBusyRef.current) return;
                void startVoiceToRhizohRef.current(keepAlive, {
                  userGestureUrgent: false,
                  langAttempt: plan.langAttempt
                });
              }, plan.delayMs);
            }
          }
        ...
        noteVoiceSttEventV0("STT_ERROR", { code: err, message: msg });
        ...
        if (err === "not-allowed" || err === "service-not-allowed") {
          ...
          speakRhizoh("Mikrofon izni gerekli. Taray─▒c─▒ ayarlar─▒ndan bu siteye izin ver.");
        } else if (err === "audio-capture") {
          fallbackToTextInput("audio_capture");
          speakRhizoh("Mikrofon bulunamad─▒ ya da kullan─▒lam─▒yor. Ses giri┼ş cihaz─▒n─▒ kontrol et.");
        } else if (err === "network") {
          ...
          if (now - Number(lastVoiceNetworkWarnAtRef.current || 0) > 15000) {
            ...
            speakRhizoh(
              "Taray─▒c─▒ ses tan─▒ma servisine ┼şu an ula┼ş─▒lam─▒yor. Bu Rhizoh ba─şlant─▒ hatas─▒ de─şil; yaz─▒p g├Ânderebilirsin."
            );
          }
        } else if (err === "language-not-supported") {
          fallbackToTextInput("language_unsupported");
          speakRhizoh("Se├ğili dilde ses tan─▒ma desteklenmiyor. Taray─▒c─▒ dilini de─şi┼ştirip tekrar dene.");
        } else if (outcome.shouldFallback) {
          fallbackToTextInput(err || "unknown");
          speakRhizoh("Ses tan─▒ma ba┼şar─▒s─▒z. Yazarak g├Ândermeyi dene.");
        }
      };
      noteVoiceSttEventV0("STT_HANDLERS_ATTACHED", {
        lang: rec.lang,
        hasOnresult: typeof rec.onresult === "function",
        hasOnstart: typeof rec.onstart === "function",
        hasOnend: typeof rec.onend === "function"
      });
      logVoiceInfoV0("STT_HANDLERS_ATTACHED", { lang: rec.lang });
      recognitionRef.current = rec;
      try {
        stampVoiceUserGestureV0("rec_start");
        rec.start();
        logVoiceWarnV0("STT_START_CALLED", { lang: rec.lang, userGestureUrgent, keepAlive });
        void ensureAmbientSound().then((audioState) => {
          logVoiceInfoV0("STT_AMBIENT_READY", { audioState });
        });
      } catch (startErr) {
        voiceSttStartInFlightRef.current = false;
        noteVoiceSttEventV0("STT_START_FAILED", { message: String(startErr?.message || startErr) });
        logVoiceWarnV0("STT_START_FAILED", { message: String(startErr?.message || startErr) });
        setMicListening(false);
        voiceNetworkRetryRef.current = 0;
        speakRhizoh("Mikrofon ba┼şlat─▒lamad─▒.");
      }
    },
    [
      speakRhizoh,
      ...
      fallbackToTextInput,
      maybeWarnVoiceSilentStopV0,
      scheduleVoiceMicRestartV0,
      gatewayUx?.phase
    ]
  );

  useEffect(() => {
    requestVoiceMicRestartRef.current = requestVoiceMicRestartV0;
  }, [requestVoiceMicRestartV0]);
  ...
  const stopVoiceLoop = useCallback(() => {
    ...
    voiceSttStartInFlightRef.current = false;
    voiceSttNoSpeechRetryRef.current = 0;
    if (voiceSttMaxRecordTimerRef.current) {
  ...
  const startVoiceLoop = useCallback(() => {
    ...
    voiceLoopStartInFlightRef.current = true;
    ...
    void (async () => {
      try {
        installVoiceSttTelemetryV0();
        const snap = await awaitVoiceAdapterRegistryReadyV0();
        if (!snap.hydrated) {
          logVoiceWarnV0("VOICE_INIT_NOT_READY", { reason: "adapter_not_hydrated" });
          speakRhizoh("Ses sistemi hen├╝z haz─▒r de─şil. Birka├ğ saniye bekleyip mikrofona tekrar bas.");
          return;
        }
        const voiceAdapter = getRhizohVoiceInputAdapterV0();
        if (!voiceAdapter || snap.fallbackMode) {
          fallbackToTextInput("no_voice_adapter");
          speakRhizoh("Ses tan─▒ma kullan─▒lam─▒yor. A┼şa─ş─▒ya yaz─▒p g├Ânderebilirsin.");
          return;
        }
        ...
        if (gw === "offline" || gw === "offline_dns") {
          setRhizohInlineError({
            title: "Rhizoh hatt─▒",
            detail: "Ge├ğit hen├╝z ba─şlan─▒yor ÔÇö konu┼şabilirsin; yan─▒t gecikebilir. Yaz─▒yla da g├Ânderebilirsin."
          });
        }
        setVoiceNetworkBlocked(false);
        voiceNetworkRetryRef.current = 0;
        ...
        voiceLoopStartInFlightRef.current = false;
      }
    })();
  ...
  const handleMicButtonClick = useCallback(() => {
    ...
    const engineBusy = voiceEngineV3Ref.current?.isBusy?.();
    if (voiceSttStartInFlightRef.current && !micListening && !engineBusy) {
      voiceSttStartInFlightRef.current = false;
      logVoiceWarnV0("MIC_REBIND_INFLIGHT_RESET", { field: rhizohFieldState });
    }

    const sessionActive =
      micListening || rhizohFieldState === "SPEAKING" || rhizohFieldState === "INTERPRETING";

    if (voiceLoopEnabled && sessionActive) {
      stopVoiceLoop();
      return;
    }
    if (voiceLoopEnabled && !sessionActive) {
      if (micListening || voiceSttStartInFlightRef.current) {
        logVoiceInfoV0("MIC_REBIND_SKIPPED", {
          field: rhizohFieldState,
          listening: micListening,
          inFlight: voiceSttStartInFlightRef.current
        });
        return;
      }
      setVoiceNetworkBlocked(false);
    ...
    const id = window.setInterval(() => {
      ...
      if (mapOk && swarm && geo?.lat != null && geo?.lon != null) {
        ...
        if (cnt >= 6) {
          pushL9SocialDraftArbitrated({
            ...
            lon: geo.lon,
            agentCount: cnt,
            text: `Istanbul Nexus'ta devasa bir enerji dalgalanmas─▒ tespiti. ${cnt} otonom ajan ana kalede savunma formasyonuna ge├ğti. #CastleGenesis`,
      ...
      if (swarm && mapOk && l9SpiralTickRef.current % 22 === 0) {
        pushL9SocialDraftArbitrated({
          ...
          lat: ISTANBUL_POI.FATIH.lat,
          lon: ISTANBUL_POI.FATIH.lon,
          text: "Atlas birimi, L10 katman─▒nda yeni bir y├Âr├╝nge geometrisi ke┼şfetti. ┼Şehir haritas─▒nda geni┼şleyen spiral deseni stabilize edildi.",
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (prevS === STATE.AGENT_STUDENT && curS === STATE.AGENT_MASTER) {
          ...
          pushL9SocialDraftArbitrated({
            ...
            lat,
            lon,
            text: `${idStr} [Prometheus] Master seviyesine ula┼şt─▒ ve kaleden ayr─▒l─▒p kendi ┼şehrini kurma yetkisi kazand─▒.`,
        ...
        if (curS === STATE.GHOSTPET && prevP === PET_STAGE.SPIRIT && curP === PET_STAGE.GUARDIAN) {
          ...
          pushL9SocialDraftArbitrated({
            ...
            lat,
            lon,
            text: `Ghost Pet ${idStr} SPIRIT a┼şamas─▒ndan GUARDIAN'a evrildi ÔÇö kale bariyerleri g├╝├ğlendirildi.`,
    ...
    const onGhost = (e) => {
      ...
      uiStore.dispatch({
        ...
        payload: {
          ts: new Date().toLocaleTimeString(),
          type: "SYS",
          data: `Ghost Pet ┬À L9 bellek nabz─▒ (idx ${idx}) ÔÇö ki┼şilik y├╝zeyi g├╝ncellendi`
        }
      });
    };
      ...
      if (d.kind === "execution_deferred") {
        uiStore.dispatch({
          ...
          payload: {
            ts,
            type: "SYS",
            data: `L9 FEEDBACK ┬À emit beklemeye al─▒nd─▒ (${d.reason || "?"}) ┬À ${String(d.trigger || "").slice(0, 24)}`
          }
        });
        ...
        uiStore.dispatch({
          ...
          payload: {
            ts,
            type: "WARN",
            data: `L9 FEEDBACK ┬À execution hold ta┼şt─▒ (32) ÔÇö ${String(d.message || "eski taslak d├╝┼ş├╝r├╝ld├╝")}`
          }
        });
      }
    };
  ...
  const handleExecute = async (overrideRaw) => {
    ...
    setLastWhy([
      "Intent ger├ğek LLM hatt─▒na y├Ânlendirildi",
      "Layer profile + runtime context ile prompt zenginle┼ştirildi",
      "Directive cevab─▒na g├Âre kamera/harita uygulanacak",
    ...
    try {
      ...
      uiStore.dispatch({
        ...
        payload: {
          ts: new Date().toLocaleTimeString(),
          type: "SYS",
          data: `RHIZOH LIVE [${out.source || "unknown"}] ┬À ${intent}`
        }
      });
      ...
      if (isBroadcastIntent) {
        try {
          ...
          speakRhizoh("Yay─▒n─▒ a├ğt─▒m. D├╝nya dinliyor.");
          const vEcho = window.setTimeout(() => speakRhizoh("─░lk yank─▒lar geliyor."), 15_000);
          ...
          uiStore.dispatch({
            ...
            payload: {
              ts: new Date().toLocaleTimeString(),
              type: "SYS",
              data: `BROADCAST_ROUTED ÔåÆ GreenRoom ┬À ${gr.traceId}`
            }
          });
        } catch (err) {
          setGreenRoomLive(null);
      ...
      const clipped = detail.length > 220 ? `${detail.slice(0, 220)}ÔÇĞ` : detail;
      setRhizohInlineError({
        title: "Rhizoh bu komutu i┼şleyemedi",
        detail: `${clipped} Ba─şlant─▒ veya oturum sorunu olabilir; birka├ğ saniye sonra tekrar deneyin.`
      });
      try {
      ...
      setRhizohMainHudReply({
        text: `Yan─▒t ├╝retilemedi: ${clipped}. A─ş ge├ğidi (├╝st durum) ve konsoldaki [Rhizoh LLM diag] sat─▒r─▒na bak─▒n.`,
        source: "error",
        at: Date.now()
      });
      speakRhizoh("Rhizoh canl─▒ hatta yan─▒t ├╝retemedi. L├╝tfen tekrar dene.");
    } finally {
      setRhizohFieldState("IDLE");
      setRealityState("WORLD_STABLE");
    }
  ...
  const handleDemoAction = async (action) => {
    if (action === "replay") {
      ...
      speakRhizoh("Bu an─▒ yeniden ya┼ş─▒yoruz.");
    ...
    if (action === "share") {
      ...
      try {
        ...
        uiStore.dispatch({
          type: "ADD_LOG",
          payload: { ts: new Date().toLocaleTimeString(), type: "SYS", data: `SHARE READY ┬À ${shareText}` }
        });
      } catch {
        uiStore.dispatch({
          type: "ADD_LOG",
          payload: { ts: new Date().toLocaleTimeString(), type: "WARN", data: "SHARE FAILED ┬À clipboard unavailable" }
        });
      }
  ...
  useEffect(() => {
    setRhizohDomainCoreUserV0(castleAuth?.user?.uid || null);
  }, [castleAuth?.user?.uid]);
    ...
    void awaitVoiceAdapterRegistryReadyV0().then((snap) => {
      ...
      try {
        if (snap.voiceRegistered && !snap.fallbackMode) {
          ...
          bootLogRef.current?.warn?.(
            "app.voice.adapter",
            `missing ÔÇö text fallback active waitMs=${waitMs} hydrated=${snap.hydrated} fallbackMode=${snap.fallbackMode}`
          );
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      console.warn("[VOICE_WATCHDOG] turn busy timeout ÔÇö releasing mic loop");
      voiceTurnBusyRef.current = false;
      voiceTurnBusySinceRef.current = 0;
      setRhizohFieldState("IDLE");
      finishVoiceTurnIfNeeded();
  ...
  useEffect(() => {
    stampVoiceUserGestureV0("app_mount");
    publishVoiceGestureAnchorToWindowV0();
    noteVoiceSttEventV0("STT_TELEMETRY_READY", {});
    return installVoiceUserGestureListenersV0();
  }, []);
  ...
  return (
    <div className="min-h-screen w-full bg-[#010103] text-white font-mono overflow-x-hidden overflow-y-auto relative select-none uppercase font-black selection:bg-cyan-400/40">
      {isWorldLayerEnabled() ? <RhizohAtmospherePresenceBridge /> : null}
      ...
      {isWorldDomainActiveV0 ? (
        <RhizohWorldDomainShellV0
          domain={activeWorldDomainV0}
          layerMode={layerModeV0}
          uiLocale={uiLocaleV0}
          activeMapTool={worldMapToolV0}
          onSelectMapTool={(id) => onApplyWorldMapToolV0(id, "WORLD_DOMAIN_MAP_STRIP")}
          spatialEngineActive={cesiumLayerActiveV0}
          onOpenGreenroom={() => onProductShellSelect("greenroom")}
          onOpenBroadcast={() => onProductShellSelect("broadcast")}
          onShareInvite={onShareWorldInviteV0}
          onModeSelect={onWorldModeSelectV0}
      ...
      {openSurfaceDrawerIdV0 && !isWorldDomainActiveV0 ? (
      ...
      {immersiveLiveTrace ? (
        <div className="pointer-events-auto fixed bottom-[5.25rem] left-1/2 z-[58] flex max-w-[95vw] -translate-x-1/2 items-center gap-3 rounded-full border border-fuchsia-400/45 bg-black/75 px-4 py-2 text-[9px] text-fuchsia-100/95 shadow-[0_0_24px_rgba(192,38,211,0.2)] normal-case">
          <span className="tracking-wide">Live room ┬À {immersiveLiveTrace.slice(0, 20)}ÔÇĞ</span>
          <button
            type="button"
      ...
      {showT0LiveChromeV0 ? (
      <div className="absolute inset-0 z-[65] pointer-events-none flex min-h-full flex-col p-4 md:p-5">
          ...
          >
            ...
            <div className="space-y-3 text-[10px] text-white/75 normal-case mt-4">
              ...
              <div className="text-white/55">
                Last created: {relationalPresenceState.sessionContinuity.lastCreated} ┬À Preference: {relationalPresenceState.sessionContinuity.preference}
              </div>
            </div>
            {greenRoomLive ? (
              <div className="mt-4 rounded-2xl border border-fuchsia-400/40 bg-gradient-to-br from-fuchsia-950/50 to-black/60 p-4 shadow-[0_0_40px_rgba(217,70,239,0.12)]">
                    ...
                    onClick={() => {
                      ...
                      uiStore.dispatch({
                        ...
                        payload: {
                          ts: new Date().toLocaleTimeString(),
                          type: "SYS",
                          data: `ENTER LIVE ROOM ┬À /greenroom/live/${greenRoomLive.traceId}`
                        }
                      });
                    }}
                  >
                ...
                {greenRoomLive.memorySealed ? (
                  ...
                  >
                    ...
                    Committed to Castle Library ┬À <span className="text-emerald-300/95">Memory sealed Ô£ô</span>
                  </div>
                ) : null}
              </div>
            ) : null}
            <div className="mt-4 rounded-2xl border border-cyan-300/30 bg-[#070d20]/80 p-4">
              ...
              <div className="text-[10px] text-white/60 normal-case">
                Confidence: {eventPreview?.confidence ?? "N/A"} ┬À Risk: {eventPreview?.risk ?? "N/A"}
              </div>
              <div className="text-[10px] text-emerald-200/90 normal-case">
                System confidence: {eventPreview?.confidence ? (eventPreview.confidence >= 0.9 ? "High" : "Moderate") : "Pending"} ┬À Governance {governanceState}
              </div>
              <div className="text-[10px] text-white/60 normal-case">
                Event: {eventPreview?.eventId ?? "pending"} ┬À Trace: {eventPreview?.traceId ?? "pending"}
              </div>
              ...
              {showWhy ? (
                <div className="mt-3 space-y-1 text-[10px] text-white/70 normal-case">
                  ...
                  <div className="text-indigo-200/90">Light route committed: knowledge ÔåÆ map ÔåÆ camera ÔåÆ voice ÔåÆ memory ÔåÆ artifact.</div>
