...
import { buildSpiralMMOAwakeningBirdPlanV0, spiralMMOAwakeningBirdHtmlV0 } from "../spiralMMOAwakeningBirdV0.js";
import { resolveSpiralMMOSixFortyFourTierLabelV0 } from "../spiralMMOPinCitizenshipV0.js";

describe("spiralMMOBirdFlockFlightV0", () => {
  it("samples helical points with depth along a spiral route", () => {
    const route = buildSpiralMMOBirdFlockRouteV0(200, 150, { seed: "test-flock", hostW: 900 });
    ...
    expect(Number.isFinite(start.pitchDeg)).toBe(true);
    expect(Number.isFinite(start.headingDeg)).toBe(true);
  });

  it("builds multiple flocks with distinct routes", () => {
    const plan = buildSpiralMMOBirdFlockPlanV0({ hostW: 900, hostH: 600, cycleSeed: 42 });
    expect(plan.flockCount).toBeGreaterThanOrEqual(2);
    const outerRadii = new Set(plan.flocks.map((f) => Math.round(f.route.outerR)));
    expect(outerRadii.size).toBeGreaterThan(1);
    expect(plan.birds.every((b) => b.routeMode === "spiral_flock")).toBe(true);
    expect(plan.birds.every((b) => !("tierShort" in b))).toBe(true);
  });

  it("labels 6+44 tiers on pins only (hour/day/month/year units)", () => {
  ...
  it("awakening bird plan has no tier badges on birds", () => {
    ...
    expect(birds.every((b) => !b.tierShort)).toBe(true);
    const html = spiralMMOAwakeningBirdHtmlV0(birds[0]);
    expect(html).not.toContain("6h");
    expect(html).not.toContain("6y");
    expect(html).not.toContain("data-rhizoh-spiral-bird-tier");
  });
});
