...
import { applyGoArenaMoveV0, getGoArenaEngineSnapshotV0, listGoArenaStonesV0 } from "./goArenaEngineV0.js";
...
/**
 * Demo move with optional KataGo confidence when sidecar is configured.
 * @param {{ x?: number, y?: number, confidence?: number, locale?: string, useKataGo?: boolean }} [opts]
 */
export async function ingestGoLearningDemoMoveAsyncV0(opts = {}) {
  const moveResult = applyGoArenaMoveV0({ x: opts.x, y: opts.y });
  if (!moveResult.ok) {
    return Object.freeze({ ok: false, reason: moveResult.reason, schema: GO_LEARNING_DEMO_INGEST_SCHEMA_V0 });
  }

  let confidence = Number.isFinite(opts.confidence) ? opts.confidence : null;
  let kataAnalysis = null;

  if (opts.useKataGo !== false && isGoKataGoGtpConfiguredV0()) {
    const snap = getGoArenaEngineSnapshotV0();
    kataAnalysis = await analyzeGoPositionKataGoV0({
      stones: listGoArenaStonesV0(),
      boardSize: snap.boardSize,
      activeColor: snap.activeColor
    });
    if (kataAnalysis?.confidence != null) {
      confidence = kataAnalysis.confidence;
    }
  }

  if (confidence == null) confidence = 0.72;

  const gate = evaluateGoLearningAgreementGateV0({ confidence, sourceCount: kataAnalysis ? 2 : 1 });
  const spacetime = buildGoSpacetimeObservationEnvelopeV0({
    locale: opts.locale,
    nodeId: "go_arena",
    channelId: "rhizoh_go_learning",
  ...
  emitSpatialTemporalTrailV0("go", {
    nodeId: spacetime.worldAnchor.nodeId,
    kind: "go_learning_move",
    payload: { boardHash: moveResult.boardHash, moveN: moveResult.moveCount, kataGo: Boolean(kataAnalysis) }
  });
  ...
  const batch = enqueueGoLearningBatchSampleV0({
    ...
    gate,
    spacetime,
    kataGoWinrate: kataAnalysis?.winrate ?? null
  });
