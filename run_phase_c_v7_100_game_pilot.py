...
def execute_phase_c_v7_pilot(num_opening_pairs=50, movetime_ms=1000, threads=2, hash_mb=256):
    ...
    pgn_path = os.path.join(root_dir, "data", "pgn_archive", "phase_c_v7_100_game_pilot_matches.pgn")
    report_path = os.path.join(root_dir, "data", "runs", "phase_c_v7_100_game_pilot_report.json")
    os.makedirs(os.path.dirname(pgn_path), exist_ok=True)

    print("=====================================================================")
    print("--- PHASE C V7: 100-GAME HALFKP + AVX2 REAL MASTER PILOT ---")
    print("=====================================================================")
    print(f"Castle Binary SHA256     : {castle_sha} (HalfKP NNUE + AVX2 SIMD + Search Code)")
    print(f"HalfKP NNUE SHA256       : {nnue_sha} (RHNNUEV4 40,960 Inputs Format)")
    ...
    with open(pgn_path, "a", encoding="utf-8") as f_pgn:
        for i in range(1, num_opening_pairs + 1):
            ...
            # Game A: HalfKP Castle White vs Stockfish Black
            g1_id = 2 * i - 1
            print(f"[PILOT V7 Match #{g1_id:03d}/{total_declared:03d}] Castle (W) vs Stockfish (B) [{fen_name}]... ", end="")
            t0 = time.time()
            ...
            f_pgn.write(f'[Event "Phase C V7 Pilot Match #{g1_id}"]\n')
            ...
            # Game B: Stockfish White vs HalfKP Castle Black (Paired Color Swap)
            g2_id = 2 * i
            print(f"[PILOT V7 Match #{g2_id:03d}/{total_declared:03d}] Stockfish (W) vs Castle (B) [{fen_name}]... ", end="")
            t0 = time.time()
            ...
            f_pgn.write(f'[Event "Phase C V7 Pilot Match #{g2_id}"]\n')
    ...
    report = {
        "benchmark_stage": "PHASE_C_V7_100_GAME_PILOT",
        "candidate_id": "RHNNUE_CANDIDATE_V7_HALFKP_AVX2",
    ...
    print("\n---------------------------------------------------------------------")
    print(f"PHASE C V7 PILOT SUMMARY: {total_valid} Valid Real Games ({elapsed_min:.2f} min)")
...
if __name__ == "__main__":
    execute_phase_c_v7_pilot(num_opening_pairs=50, movetime_ms=1000, threads=2, hash_mb=256)
